...
// The render surface (QuestionVisualAid) shows these as <img>, in source order.
//
// SPLIT — BANK-1 PR-1. The two arrays now live in their own files so the Maths
// and Science figure-binding lanes can edit figures without sharing one path
// (lane_overlap compares PATHS, not symbols, so one file meant one lane at a
// time). They are imported and re-exported here UNCHANGED: every existing
// importer of this module is untouched, and getFiguresForQuestion below
// concatenates them in the same order, with the same filter and return type.
// Proved behaviour-neutral rather than asserted — getFiguresForQuestion was
// dumped for all 128 bound question ids before and after the move and the two
// dumps are byte-identical.
//
// ⚠ A RE-EXPORT IS INVISIBLE TO A CONSUMER THAT DOES NOT IMPORT.
// scripts/ops/tutor_visual_catalogue_acceptance.mjs regex-parses figure entries
// out of the SOURCE TEXT, so it never sees this re-export; it was updated in the
// same commit to read the two files below. See [FU-NON-IMPORTING-CONSUMERS].
// =============================================================================
import { MATHS_FIGURE_VISUALS } from "./figures/mathsFigureVisuals";
import { SCIENCE_FIGURE_VISUALS } from "./figures/scienceFigureVisuals";

export { MATHS_FIGURE_VISUALS, SCIENCE_FIGURE_VISUALS };
