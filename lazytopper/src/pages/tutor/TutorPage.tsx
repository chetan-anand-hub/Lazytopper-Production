...
// ConceptTeachDrawer / TutorDrawerV2. Stage 1 = the chat shell: a nameless, warm,
// board-shaped, fully-responsive Claude-style chat that knows the student (reads
// MI/progress into a brief) and opens on continuity + the diagnostic fork.
//
// Two-panel scaffold (D-TUT-13): the chat panel is primary and full-width; the
// explanation panel is built as a CLOSED scaffold here (nothing to show in Stage 1)
// so Stage 3 (visuals) slots in without restructuring. Split on desktop / overlay
// on mobile — handled purely in CSS (one responsive component, no JS width branch).
//
// Bare full-screen route (App.tsx BARE_FULLSCREEN_PREFIXES): the tutor owns its
// whole viewport and its own chrome (back + nameless header + language). Product
// grammar: light, green hsl(152,55%,45%), Fraunces + Inter.
...
// exam content stays English (enforced in the system prompt). Real selector, not a
// fake chip — the value is passed through to the model.
const LANGUAGES = [
  "English",
  "Hindi",
  "Tamil",
...
export default function TutorPage() {
  ...
  const { opener, messages, status, error, started, send, retry } = useTutorSession({
    uid,
      ...
      <div className="lt-tutor__body">
        ...
        <section className="lt-tutor__chat">
          <div className="lt-tutor__stream" ref={streamRef} role="log" aria-live="polite">
            {/* Opener: warm orientation + the diagnostic fork (no fabricated continuity) */}
            <div className="lt-tutor__t">{opener.text}</div>
            {!started && (
            ...
            {messages.map((m, i) =>
              m.role === "tutor" ? (
                <div key={i} className="lt-tutor__t">
                  {m.content}
                </div>
              ) : (
        ...
        {/* Explanation panel — CLOSED scaffold in Stage 1 (nothing to show yet).
            Stage 3 populates it (NCERT page / figure / interactive). Split on
...
.lt-tutor__chat { flex: 1; min-width: 0; display: flex; flex-direction: column; }
.lt-tutor__stream {
  flex: 1;
  overflow-y: auto;
