...
const HIGHLIGHT_BRIDGE = `<script id="lt-hl-bridge">(function(){function ensureStyle(){if(document.getElementById('lt-hl-style'))return;var s=document.createElement('style');s.id='lt-hl-style';s.textContent='@keyframes lt-hl-glow{0%,100%{box-shadow:0 0 0 3px rgba(251,191,36,0.9),0 0 18px rgba(251,191,36,0.5)}50%{box-shadow:0 0 0 6px rgba(251,191,36,1),0 0 32px rgba(251,191,36,0.8)}}.lt-hl-active{outline:2px solid #fbbf24!important;outline-offset:3px!important;animation:lt-hl-glow 0.9s ease-in-out 4!important;border-radius:6px!important;}';document.head.appendChild(s);}window.addEventListener('message',function(ev){var d=ev.data;if(!d||d.type!=='lt-highlight')return;ensureStyle();var kws=(d.keywords||[]).map(function(k){return(k+'').trim().toLowerCase();}).filter(function(k){return k.length>2;});if(!kws.length)return;var all=document.querySelectorAll('*');var matched=[];for(var i=0;i<all.length;i++){var el=all[i];if(el===document.body||el===document.documentElement)continue;if(el.children.length>10)continue;var txt=[(el.id||''),(el.getAttribute('title')||''),(el.getAttribute('data-label')||''),(el.getAttribute('aria-label')||''),(el.children.length===0?(el.textContent||'').substring(0,150):'')].join(' ').toLowerCase();if(kws.some(function(k){return txt.indexOf(k)>=0;})){matched.push(el);}}matched=matched.slice(0,6);matched.forEach(function(el){el.classList.add('lt-hl-active');setTimeout(function(){el.classList.remove('lt-hl-active');},3500);});});})();<\/script>`;
