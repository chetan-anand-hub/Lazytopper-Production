...
describe("runtime-data dependence — the input no source fingerprint can cover", () => {
  /**
   * ★★ THE CONTROL FOR THE SHARPEST GUARD IN THIS STEP. A page whose DOM changes when the
   * app's API is unreachable has a captured copy that is a photograph of one response: it
   * drifts the moment the data changes, and every other guard here — the entry-chunk
   * fingerprint, the asset-existence check, the coverage check — stays green while it does.
   * Rendering twice and comparing the OUTPUT is the only way to see it.
   */
  it("REJECTS a page whose DOM differs when the app API is unreachable", () => {
    const base = capture("/exam-trends", "Exam trends");
    const failures = validateCaptures([
      { ...base, htmlWithApis: `${base.html}<aside>live data</aside>` },
    ]);

    expect(failures).toHaveLength(1);
    expect(failures[0]).toContain("DEPENDS ON A RUNTIME API RESPONSE");
    expect(failures[0]).toContain("/exam-trends");
  });

  it("names the endpoints the offending page called, so the cause is in the message", () => {
    const base = capture("/exam-trends", "Exam trends");
    const failures = validateCaptures([
      {
        ...base,
        htmlWithApis: `${base.html}<aside>live</aside>`,
        apiCalls: ["/api/exam-trends-live"],
      },
    ]);

    expect(failures[0]).toContain("/api/exam-trends-live");
  });

  /**
   * ★ THE OTHER HALF OF THE CONTROL, AND IT IS WHY THIS GUARD COMPARES OUTPUT RATHER THAN
   * COUNTING REQUESTS. Measured on the real build: EVERY advertised page requests
   * `/api/cbse-exam-date` (App.tsx's daily pace-profile init runs on every route), and the
   * DOM is byte-identical with that call aborted — on all 58. A rule of "fail if a page
   * calls an app API" would therefore have failed every page on day one, over a call that
   * feeds no rendering.
   */
  it("ACCEPTS a page that calls an app API without its output depending on the response", () => {
    const base = capture("/topic-hub/trigonometry", "Trigonometry ratios");
    const failures = validateCaptures([
      { ...base, htmlWithApis: base.html, apiCalls: ["/api/cbse-exam-date"] },
    ]);

    expect(failures).toEqual([]);
  });

  it("is not vacuous when the comparison run is absent", () => {
    // A capture with no control run recorded must not silently pass the check by
    // having nothing to compare against — it simply is not asserted here, and the
