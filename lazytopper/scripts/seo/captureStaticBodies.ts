...
async function main(): Promise<void> {
  ...
  // ★ THE MANIFEST IS THE STALENESS FINGERPRINT, AND IT IS ONE SMALL FILE ON PURPOSE.
  // It records the entry chunk this capture was taken against. That is a sound
  // fingerprint for "the source has moved since", verified rather than assumed: the
  // entry chunk embeds the content-hashed filenames of its lazy children — 76 of them,
  // including `noteSpecRegistry-*.js` and `canonicalQuestionBank-*.js` — so editing a
  // note spec or the question bank renames that child, which changes the entry's bytes,
  // which moves the entry's own hash. Content cannot drift without this moving.
  //
  // ⚠ It is NOT a timestamp. git does not preserve mtimes and a fresh checkout stamps
  // every file identically, so a timestamp guard is green on a clean clone no matter how
  // stale the content is — a control that cannot fail.
  const entry = entryChunkOf(outDir);
  writeFileSync(
    join(PRERENDERED_DIR, "manifest.json"),
    `${JSON.stringify(
      {
        note:
          "Generated by `pnpm seo:capture`. Do not edit by hand. `entry` is the staleness " +
          "fingerprint: if the build's entry chunk no longer matches, this artifact was " +
          "captured from different source.",
        entry,
