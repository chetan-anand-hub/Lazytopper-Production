...
const batchPart = parseInt(process.argv[4] || "0");
...
if (existsSync(outPath) && batchPart === 0) {
  console.log(`SKIP: ${packFile} already exists`);
  process.exit(0);
}

const half = Math.ceil(ch.need / 2);
const batchSize = batchPart === 0 ? half : ch.need - half;
const startIdx = batchPart === 0 ? 1 : half + 1;

const prompt = `Generate exactly ${batchSize} CBSE Class 10 ${subj} questions for "${ch.key}". IDs from ${ch.prefix}-${String(startIdx).padStart(3,"0")}. Subtopics: ${ch.subtopics.join(",")}. NCERT: ${ch.ncertExs.join(",")}.

Rules: ~30%Easy ~45%Medium ~25%Hard. 50%+ competency-based.${batchPart===0 ? " Include 3 Assertion-Reasoning, 4 PYQs (2019-2025), 2 Case-Based (4m)." : " Include 3 Assertion-Reasoning, 4 PYQs (2019-2025), 1 Case-Based (4m), 3 Long(5m)."} Rest MCQ/Short. Tag ncertRef.

JSON array only. Schema: {"id":"","subject":"${subj}","topicKey":"${ch.key}","subtopic":"","section":"A|B|C|D|E","marks":1,"format":"MCQ|Short|Long|Case-Based|Assertion-Reasoning","difficulty":"","bloomSkill":"","questionText":"","options":[],"answer":"","solutionSteps":[],"finalAnswer":"","isCompetencyBased":false,"pyqYear":null,"pyqSet":null,"ncertRef":null}
A-R options:["Both A and R are true, and R is the correct explanation of A.","Both A and R are true, but R is not the correct explanation of A.","A is true, R is false.","A is false, R is true."]
Non-MCQ: options=[]. JSON only.`;

console.log(`Gen batch${batchPart} (${batchSize}Qs) for ${subj}/${ch.key}...`);
...
try {
  ...
  if (bi >= 0 && li > bi) {
    try { questions = JSON.parse(text.substring(bi, li + 1)); } catch (e2) {
      const lastObj = text.lastIndexOf("}");
      if (lastObj > bi) {
        try { questions = JSON.parse(text.substring(bi, lastObj + 1) + "]"); } catch (e3) { throw e; }
      } else throw e;
    }
  } else throw e;
}

console.log(`Got ${questions.length} questions for batch${batchPart}`);

if (batchPart === 0) {
  writeFileSync(outPath + ".part0.json", JSON.stringify(questions));
  console.log(`Saved part0 (${questions.length})`);
} else {
  let part0 = [];
  const p0Path = outPath + ".part0.json";
  if (existsSync(p0Path)) {
    part0 = JSON.parse(readFileSync(p0Path, "utf8"));
  }
  const all = [...part0, ...questions];
  const exportName = `${ch.prefix}_PACK${ch.packNum}`;
  const tsContent = `import type { CanonicalQuestion } from "../../types";\n\nexport const ${exportName}: CanonicalQuestion[] = ${JSON.stringify(all, null, 2)};\n`;
  writeFileSync(outPath, tsContent);
  try { require("fs").unlinkSync(p0Path); } catch {}
  console.log(`DONE: ${packFile} (${all.length} questions total)`);
}
