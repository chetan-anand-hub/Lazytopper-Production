...
async function genBatch(batchNum, batchSize, startId) {
  const prompt = `Generate exactly ${batchSize} CBSE Class 10 ${subj} questions for "${ch.key}". IDs: ${ch.prefix}-${String(startId).padStart(3,"0")} onward. Subtopics: ${ch.subtopics.join(",")}. NCERT: ${ch.ncertExs.join(",")}.

~30%Easy ~45%Medium ~25%Hard. 50%+ competency-based. Include ${batchNum===0?3:3} Assertion-Reasoning(marks:1), ${batchNum===0?4:4} PYQs(2019-2025), ${batchNum===0?2:1} Case-Based(4m), ${batchNum===0?1:2} Long(5m). Rest MCQ/Short. Tag ncertRef.

JSON array only:{"id":"","subject":"${subj}","topicKey":"${ch.key}","subtopic":"","section":"A|B|C|D|E","marks":1,"format":"MCQ|Short|Long|Case-Based|Assertion-Reasoning","difficulty":"","bloomSkill":"","questionText":"","options":[],"answer":"","solutionSteps":[],"finalAnswer":"","isCompetencyBased":false,"pyqYear":null,"pyqSet":null,"ncertRef":null}
A-R options:["Both A and R are true, and R is the correct explanation of A.","Both A and R are true, but R is not the correct explanation of A.","A is true, R is false.","A is false, R is true."] Non-MCQ:options=[]`;
  ...
  try { return JSON.parse(text); } catch {
    ...
    if (bi >= 0 && li > bi) {
      try { return JSON.parse(text.substring(bi, li + 1)); } catch {
        const lo = text.lastIndexOf("}");
        if (lo > bi) {
          try { return JSON.parse(text.substring(bi, lo + 1) + "]"); } catch (e) { throw e; }
        }
      }
    }
...
const rest = ch.need - half;

console.log(`Generating ${ch.need} Qs for ${subj}/${ch.key} in 2 batches...`);

const [b0, b1] = await Promise.all([
  genBatch(0, half, 1),
  genBatch(1, rest, half + 1),
]);

console.log(`Batch0: ${b0.length}, Batch1: ${b1.length}`);
const all = [...b0, ...b1];
...
console.log(`DONE: ${packFile} (${all.length} questions)`);
