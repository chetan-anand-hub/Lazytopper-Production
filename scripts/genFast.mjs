...
const prompt = `Generate ${n} CBSE Class 10 ${subj} questions for "${ch.key}". Subtopics: ${ch.subtopics.join(",")}. NCERT: ${ch.ncertExs.join(",")}.

Rules: ~30% Easy, ~45% Medium, ~25% Hard. 50%+ competency-based. Include 6 Assertion-Reasoning (A/R format, marks:1), 8 PYQs (2019-2025), 3 Case-Based (marks:4), 3 Long (marks:5). Rest: MCQ/Short. Tag ncertRef where applicable.

Return ONLY JSON array. Each: {"id":"${ch.prefix}-XXX","subject":"${subj}","topicKey":"${ch.key}","subtopic":"...","section":"A|B|C|D|E","marks":1,"format":"MCQ|Short|Long|Case-Based|Assertion-Reasoning","difficulty":"Easy|Medium|Hard","bloomSkill":"Remembering|Understanding|Applying|Analysing|Evaluating","questionText":"...","options":[],"answer":"...","solutionSteps":["..."],"finalAnswer":"...","isCompetencyBased":false,"pyqYear":null,"pyqSet":null,"ncertRef":null}

MCQ/A-R: options=4 choices. A-R options=["Both A and R are true, and R is the correct explanation of A.","Both A and R are true, but R is not the correct explanation of A.","A is true, R is false.","A is false, R is true."]. Others: options=[].
JSON only, no markdown.`;

console.log(`Generating ${n} Qs for ${subj}/${ch.key}...`);
...
try {
  ...
  if (bracketIdx >= 0 && lastBracketIdx > bracketIdx) {
    questions = JSON.parse(text.substring(bracketIdx, lastBracketIdx + 1));
  } else {
    throw e;
  }
}
...
console.log(`DONE: ${packFile} (${questions.length} questions)`);
