...
HANDOFF-VERIFIED 2026-09-11 with `git log e0d17da1..fa3662db` (exactly four commits, all merged by the owner:
2f623e01 = #760 WS-1 12:43:10 · 0aebd47d = #761 FIG-SCI-1 12:51:40 · 674121e7 = #762 docs 13:31:49 ·
fa3662db = #763 PR-3 13:43:16, all +05:30).
RE-DERIVE IT BEFORE ACTING:  git ls-remote origin base/approved-thru-437
OPEN PRs AT THE TIME OF WRITING: #764 DRAFT — rulings 1–4 content PR (lane/content-rulings-1-4, head d5f85b2c,
   rebased on fa3662db) — verified with `gh pr list --state open` at writing; a status claim — re-check, and
   check EVERY open PR's file list (lane_overlap compares exact paths against ALL open PRs, bot PRs included).

WHAT #760 DID (WS-1): Worksheets can offer every board-assessed Science chapter — `heredity`,
   `magnetic-effects-of-electric-current`, `human-eye-and-colourful-world` restored to worksheetModel.ts
   SCIENCE_TOPICS_RAW (13 chapters, was 10); DELETED_TOPIC_KEYS emptied; a SECOND defect fixed in the same
   file: SCIENCE_KEY_TO_TREND_KEY lacked the three keys ⇒ a restored chapter would have drawn weight 1 in
   board-weightage mode. 682 rows (243/222/217) were unreachable from Worksheets. Two existing suites now
   assert the OPPOSITE; NEW topicReachability test (14) derives the expected set from topics.ts + the bank;
   mutation-proven. Gates all PASS (worksheet vitest 98/98; both matrices; CI green). Pin file untouched.
   ⚠ LIVE-VERIFY OWED: /practice/worksheets lists 13 Science chapters — [FU-WS-1-LIVE-VERIFY-OWED].

WHAT #761 DID (FIG-SCI-1): 109 science figures bound (Item Bank 44 incl. 11 printed tables · Exemplar 34 ·
   NCERT textbook 7 · CFPQ 24), every webp sha256-matched to its manifest; SCIENCE_FIGURE_VISUALS 48 → 157;
   rows with ≥1 bound figure 186 → 295; figure-demanding bound 183 → 292 / unbound 310 → 201; publishable
   unchanged at 2,982 AT THAT POINT (pre-#763). NEW scienceFigureVisuals.reachability.test.ts pinned at 109,
   mutation-proven. NOT bound, by design: 9 draw-it rows (source prints no figure; CBE-S-MNM-B-004's image is
   missing from the PDF itself), 3 decorative photographs (CBE-S-CHEM-B-003, CFPQ-S-ABS-013, CFPQ-S-EYE-010),
   METAL-NCERT-3-VSA-006 (stem names a different table). Crops carry fragments of the SOURCE's own watermark;
   only neighbouring questions' text was whitened. Gates: both matrices PASS, build + verify-production-build
   PASS, 4 probed assets ship in artifacts/lazytopper-app/dist/public/app/figures/..., decorative control
   absent; CI green. ⚠ LIVE-VERIFY OWED: one Electricity + one Light question renders its figure on
   Practice / Chapter Test / Full Mock — [FU-FIG-SCI-1-LIVE-VERIFY-OWED]. HPQ / Mock Paper do NOT read the bank.

WHAT #763 DID (PR-3): Rule 5 of isPublishable now consults the figure binder (defaultHasBoundFigure via
   visualConceptRegistry); the guard test's 12 exact literals became floors / ceilings / partition identities
   + a negative control; discrimination proven three ways; both matrices PASS; the ONLY importer of
   publishability.ts is the guard test ⇒ nothing rendered changes, no live-verify.
   ★ LIVE, MEASURED at fa3662db (CONTROLLER-RECORDED, independent seat, relayed by the owner): publishable
   2,982 → 3,212; reasons {ai-generated-source 2952, unmarked-step 2336, requires-absent-figure 162}; rows
   with a bound figure 295 (maths 158, science 157); bogus id → 0. Partition 2952+2336+162+3212 = 8662.
   NOT the lane's 3,144: that was measured at a157c741, BEFORE #761's 109 bindings — on trunk the escape
   frees 230 (392 → 162). Both correct on their base. THE PIN-FILE SERIALISATION IS OVER: #764's guard test
   passed 38/38 after rebasing with NO pin edit. [FU-PR3-L560-GAP-HAS-NO-DIRECTION] stays OPEN (ruling on a
   literal bound for the L560 gap still pending; the identity is what is live).

★ NEXT, IN ORDER (the controller's sequence, ruling 10):
   1. OWNER MERGES #764 (draft): 73 requiresDiagram flips = 53 Z3 unbound + SP-M-2022-TRIG-E-001 + 9 maths
      + 10 science; CG pair CBE-M-CG-A-001 / CG-B-002 → stems corrected to the printed figure A(−2,2)
      B(−1,−2) C(3,0), the Item Bank p230 crop BOUND, flags stay true, maths pin 88 → 90 — per the OWNER
      RULING this afternoon (corrected stems + bound figure), which SUPERSEDES a relayed "flag off, don't
      bind" reading; 6 regex-only rows listed for review, not edited; guard test 38/38, pin file untouched.
   2. CONTROLLER PUSHES STEPMARK batches 5 and 6 (rank-1 / rank-2 topics by addressable count, scheme-based
      allocation, RECOVER-1 skip honoured) — no longer gated on the pin file.
   3. FIG-SCI-2 — 108 rows: worksheets / Foundation / chapterwise / PYQ / SQP science (CONTROLLER-RECORDED
      split gdr 35 · fnd 17 · chapterwise 15 · pyq 19 · preboard 18 · sqp 2 · other 2). Same method as
      FIG-SCI-1; post-#763 each binding of a step-marked row MOVES the publishable floor — state the
      measured delta in the lane's §0, derived through rule order on the base you land on.
   4. WAVE 4 transcription per the SUPPLY PLAN (per-chapter gap table + on-disk source plan, in build):
      STEPMARK per chapter (1,890 annotatable) · transcription per chapter from the PDFs · AI-pack
      retirement per topic once its human supply lands · fresh skeptic per PR.
   THEN wave 5 authoring → HPQ-2027 refresh → guards (unchanged from the TIERMAP-1 banner).

⚠ COORDINATION — a COFOUNDER SESSION runs the SEO lanes (SEO-CANONICAL-1 dispatched: a post-build script
   under lazytopper/scripts/seo/, package.json, possibly vercel.json and src/config/sitemapUrls.ts). Tell it
   BEFORE any push touching index.html, vercel.json, lazytopper/package.json, src/config/sitemapUrls.ts,
   src/config/canonicalUrl.ts, src/config/head.guard.test.ts, or adding / moving files in
   lazytopper/scripts/seo/. Stale remote branch lane/engine-1-ten-pages (retired /questions arc) is flagged
   for closure — branch deletion is an OWNER action, never an agent's. Full note: OPEN_QUESTIONS, 2026-09-11
   WS-1 + FIG-SCI-1 + PR-3 section, `### COORDINATION`.

★ LESSONS OF WAVE 1 (CONTROLLER-RECORDED — carry them into every brief):
   · a predicted delta must be re-derived THROUGH rule order (+162 not +183) — run the predicate, never
     subtract — and ON THE BASE YOU LAND ON (+230 at fa3662db, not +162 at a157c741);
   · `scope:guard --mode product` fails-by-classification on lazytopper/scripts diffs → `--mode mixed`;
   · the main checkout's node_modules is dangling → each lane installs its own;
   · a 429 session limit kills every subagent at once; worktrees keep partial edits; a RESUME brief that
     reconciles `git diff` against the spec recovers without redo — commit locally as soon as verified;
   · deterministic topic ranking (count DESC, key ASC) prevents sibling builders colliding.

NEW FUs (each with its own heading on the follow-ups board, 2026-09-11 WS-1 + FIG-SCI-1 + PR-3 section):
   [FU-WS-1-LIVE-VERIFY-OWED] · [FU-FIG-SCI-1-LIVE-VERIFY-OWED] · [FU-PR3-L560-GAP-HAS-NO-DIRECTION] ·
   [FU-21-BOUND-ROWS-STILL-UNMARKED]. UPDATED IN PLACE: [FU-WORKSHEETS-RETIRED-TWINS-OLD-KEYS] (WS-1 merged;
   the twins' dead keys remain), [FU-METAL-NCERT-3-VSA-006-STEM-NAMES-WRONG-TABLE] and
   [FU-CFPQ-S-ENV-005-DIAGRAMDESCRIPTION-MISMATCH] (FIG-SCI-1 merged; both still open).

The TIERMAP-1 banner below is superseded ONLY in its trunk SHA, its open-PR line and its NEXT list
(steps 1, 2 and 4 of that list are DONE — #760, #763, #761; step 3 is draft #764). Its substance stands.
```
...
# Updated: 2026-08-25 (post-**WAVE MI-INTEGRITY-3**, docs-only lane `HANDOFF-MI9`. Trunk `65b6c0e2b1973d4042a383931a25d66c9330b95c` — **re-derived with `git ls-remote`, and re-derived a SECOND time after a mid-lane spec reissue; it did NOT move during this wave.** **FOUR PRs ON TRUNK — `#699` `f34c395b` · `#700` `ec97f743` · `#701` `c587a4ae` · `#702` `29220c44`.** ★★★ **THE HEADLINE: THE GRADER ARC'S FOUR FIXES WERE ON TRUNK WITH NO HANDOFF ENTRY, AND THE RECORD ACTIVELY SAID THEY HAD NOT SHIPPED** — their lane names appeared only as *"awaiting owner commit + push"*, *"HOLD"* and *"blocked on 3 owner rulings"*, which is worse than absence. ⚠ **The dispatch's own phrasing "their changes appear in NO handoff" was itself literally FALSE and the lane corrected it.** **WHAT A STUDENT GOT: a chapter is a weakness only on EVIDENCE (`#699`) — every chapter had qualified for every student permanently, a new student scored 45 and A PERFECT-RECORD STUDENT STILL SCORED 30 against a threshold of 5, because a retired page's silence was scored as ignorance, AND PARENTS SAW IT; a right answer reached by broken reasoning is now visible to MI and the tutor (`#700`), where `stepDetails` had been ALWAYS EMPTY for objective questions, right and wrong alike; a student who loses marks is told WHY and one who catches their own slip KEEPS THE MARKS (`#701`) — THE PRODUCT HAD BEEN PUNISHING SELF-CORRECTION; and the grader can finally be measured (`#702`), 8 rules to 16 with TEN ARRIVING BY IMPORT, NOT COPY, so the harness cannot drift from the grader by transcription.** ★★★ **THE MOST VALUABLE OUTPUT OF THIS WAVE WAS A LANE THAT REFUSED TO START: `HANDOFF-MI9` v1 was BLOCKED at its own §0c.0 premise gate — its ledger carried THREE columns where `premise_ledger_check.mjs` requires SIX, so THE CHECKER RETURNED EARLY AND NO PREMISE ROW WAS EVER ANCHOR-CHECKED.** ⇒ ★★ **The four column errors were ALL TRUE and still UNDER-DESCRIBED what the run established, which was nothing whatever about the premises — SPECIMEN 11 FIRED ON THE GATE'S OWN OUTPUT.** **CONTROL: `ops/AGENT_SPEC_TEMPLATE.md` fails at ROW level having PASSED the column stage, proving the stage is passable and that v1 specifically failed it.** ⇒ **RULE, NOW STANDING: AUTHORS ARE NOT EXEMPT FROM THE GATE THEY COMMISSIONED — and a gate the author ran is not a gate you ran; the lane re-ran v2's gate independently (`10 premises · 7/7 anchors RESOLVED · 0 UNCHECKED · 3 UNVERIFIED · exit 0`).** ★★★ **FOUR CONTROLLER PREMISES WERE OVERTURNED FROM BELOW: the ledger shape; `CLAUDE.md` §10 (v1 said "expect NO hit in §10" — FALSE, `CLAUDE.md:241` INSIDE §10 says *"log it in `DECISION_LOG`"* ⇒ **§10 IS INTERNALLY INCONSISTENT: IT DIRECTS YOU TO LOG TO THE VERY FILE IT OMITS FROM ITS OWN UPDATE LIST**, a sharper indictment than "it omits it"); the mojibake blindness; and an INVENTED WIRE-2 contradiction (the record is UNANIMOUS ACROSS NINE RESTATEMENTS — B2 STRUCK).** ⇒ **The instruction that produced all four is "RE-DERIVE IT, DON'T INHERIT IT — IF YOUR FINDING DIFFERS FROM THE CONTROLLER'S, YOURS WINS."** ★★★ **B1 SETTLED BY TWO-SIDED CONTROL: `check:mojibake` DOES REACH `handoff/`. `repoRoot` is `git rev-parse --show-toplevel`, NOT `lazytopper/`, and `handoff/` sits in `REPORT_ONLY_PREFIXES` — SCANNED, COUNTED AND PRINTED ON EVERY RUN, MERELY NOT ENFORCED.** Injection into `handoff/CURRENT_STATE.md` moved report-only hits `17 -> 18` at exit 0; injection into the enforced `ops/AGENT_STANDING_RULES.md` moved enforced hits `0 -> 1` at **exit 1**; both reverted, removal proven, baseline re-measured identical. ⇒ ⛔ **`ops/CONTROLLER_SUBAGENT_MODEL.md:373` IS FALSE ON TRUNK — "STRUCTURALLY BLIND" IS NOT "REPORT-ONLY", the claim was true of an older revision, was fixed under `[GUARD-3]`, and SURVIVED AS A RESTATED FACT IN THREE DISPATCHES AFTER IT STOPPED BEING TRUE.** ★★ **SPECIMENS 9-15 ARE NOW IN `ops/AGENT_STANDING_RULES.md`, WHICH CONTAINED ZERO SPECIMENS BEFORE THIS LANE** — sourced from the CANONICAL table, the same close-out's EARLIER table being SUPERSEDED with **11 and 12 TRANSPOSED**, which is itself specimen 11: two true lists presented as one. ⚠ **THREE GOVERNING-DOC CORRECTIONS ARE FOLLOW-UPS, NOT EDITS — `ops/CONTROLLER_SUBAGENT_MODEL.md:373` and `:181`, and `CLAUDE.md` §10 — all FORBIDDEN to a docs lane; THE OWNER RULES ON WHO CORRECTS A GOVERNING DOC.** ⚠ **`[FU-CLAUDEMD-S10-OMITS-DECISION-LOG]` now raised by FIVE lanes.** ⚠ **NEW: `[FU-MOJIBAKE-HANDOFF-DRIFT-UNGATED]` — the scanner's comment says "the residual 8" and the gate reports 17 across 6 files; `handoff/` is REPORT-ONLY so NOTHING EVER GOES RED on that drift.** ⚠⚠ **THE OWNER'S ERRATUM SLOT IS RESERVED AND EMPTY — six GRADER-LIVE-DEFECTS-SCOUT premises were disproved, four of them wrong statements about the repo, and THE OWNER WRITES THAT TEXT HIMSELF; a controller never authors an erratum and neither does a lane.** **`[FU-DPDP-GUARDIAN-CONSENT]` is STILL UNRULED and LAUNCH-BLOCKING.** **`[FU-SHEET-DORMANT-NO-CONSUMER]` STAYS OPEN — `#682` is on trunk but BEING ON TRUNK IS NOT BEING TRIGGERED.**)
# Previously: 2026-08-24 (post-**WAVE MI-INTEGRITY-7 + WAVE MI-INTEGRITY-8**, docs-only lane `HANDOFF-MI8`. Trunk `deddf595` — ⚠ **trunk MOVED FOUR TIMES across the two waves: `fd85835e` → `6260db7c` → `f75e8356` → `deddf595`.** **THREE CONTROLLERS, NINE UNITS, FOUR PRs ON TRUNK — `#694` · `#695` · `#696` · `#697`, ALL MERGED AND ALL VERIFIED BY `gh pr view --json mergeCommit` + `git log -- <path>`.** ⚠⚠ **ONE ENTRY COVERS TWO WAVES BECAUSE WAVE MI-INTEGRITY-7 CLOSED WITHOUT A HANDOFF PR**, and ⚠⚠ **CONTROLLER A's CLOSE-OUT IS STALE ON ITS OWN CENTRAL FACT — it says *"NOTHING OF MINE IS ON TRUNK"* and lists A3 as NOT PUSHED, and BOTH ITS PRs MERGED. Derive lane tables from `gh`/`git`, never from a close-out.** ⛔ **NO CONTROLLER C CLOSE-OUT EXISTED ON DISK — nothing in this handoff is C's content and none was invented on C's behalf.** ★★★ **THE HEADLINE: THE GRADER ARC'S LAST WAVE SHIPPED FOUR PRs AND STOPPED A FIFTH, AND THE MOST VALUABLE THING IT PRODUCED WAS THE LANE THAT REFUSED TO BUILD** — **FIVE units returned HOLD with ZERO files and every refusal was correct.** ⚠⚠⚠ **THE FINDING THAT OUTWEIGHS THE FIXES: losing `AI_PROVIDER` ALONE, with a valid API key present, flipped production into fabricating grades — HTTP 200, a fabricated 70%, an invented sentence about a student's own working, all flowing into Mistake Intelligence, the store the tutor reads. ONE ENVIRONMENT VARIABLE WAS THE WHOLE DISTANCE BETWEEN THIS PRODUCT AND INVENTING MARKS FOR CHILDREN. `#695` closes it** — ⚠ **and the live-verify must UNSET THE VARIABLE AND RESTART; `resolveConfig` runs ONCE AT BOOT, so revoking a key at runtime tests the wrong thing and comes back falsely clean.** ★★★ **AND TWICE A SCREENSHOT FOUND A DEFECT AFTER EVERY GATE WAS GREEN — once with 199 green assertions behind it, and once where ALL FOUR `PracticePage` SUITES WERE 35/35 GREEN BEFORE AND AFTER THE FIX.** ⇒ **A FIX REMOVES A DEFECT; CLOSING THE HOLE REMOVES THE REASON IT SURVIVED.** ⚠⚠ **THE COST, OWNER-RULED NOT TO BE SMOOTHED: `#696` KNOWINGLY SHIPS A THIRD COMPONENT OVER ONE SHAPE, so `[FU-READ-SHEET-TWO-PATTERNS]` is LARGER after this lane than before it — CHOSEN, NOT OVERLOOKED.** ⚠⚠ **U3 `TAXONOMY-3BUCKET` IS SPECIFIED (`15547 / 5D4DC3B95304`; `168944EC3C03` and `70CD15ED1062` ARE DEAD), REPAIRED ON FOUR DEFECTS, AND NOT BUILT — stopped by a HARD COLLISION with Controller C on its PRIMARY TARGET, `mistakeIntelligence.ts`. SIXTH TIME THE DISJOINTNESS ASSUMPTION HAS BEEN WRONG IN THIS ARC.** **NEXT, RULED WITH ITS REASON: C's `MI-INTAKE-FILTER` merges FIRST, THEN U3, NOT MERGED INTO ONE LANE** — *MI-INTAKE is one predicate at `:147`; U3 is ~52 sites across 19 files; merging makes a small verifiable change hostage to the largest lane of the arc.* ★★ **CARRY THE SATURATION CAVEAT OR A READER TAKES AN INERT SURFACE FOR A FAILED LANE: three clauses in `weakAreaAggregator.ts` fire on ABSENCE against a threshold of 5 and the mastery store has NO LIVE WRITER ⇒ ALL 26 CHAPTERS QUALIFY FOR EVERY STUDENT; the weak-area list is a CONSTANT today, and NOBODY MAY LIVE-VERIFY U3 BY LOOKING AT IT.** ⚠ **COUNTS READ FROM TRUNK'S OWN CI RUN, NEVER HARDCODED: vitest `145 files / 1882 tests`; root matrix `30 suites / 206 checks`; ops matrix `8 suites / 24 checks`.** **`[FU-DPDP-GUARDIAN-CONSENT]` is STILL UNRULED and LAUNCH-BLOCKING.**)
