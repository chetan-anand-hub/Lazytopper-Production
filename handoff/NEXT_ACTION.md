...
> - **IMMEDIATE NEXT (owner-picked):** the CT fast-follows are DONE (#380); the **Full Mock rebuild is DONE + FINALIZED** (#387 + #391 — built, LINKED, cross-device closed; see the bullet above). Remaining: (1) **Bank extraction / expansion** — 4 batches merged (#381/#384/#385/#388; ≥75-distinct scarce floor; next Science topic per `BANK_EXPANSION_LANE_STATE`); case-based is a Fable AUTHORING lane (Z3), not extraction. (2) **Me/Progress redesign (arc PR-4)** — now the surface critical path. (3) **[FU-CT-BALANCED-MIX]** (wire the CT draw to `drawBalancedSet`) when the owner releases it.
