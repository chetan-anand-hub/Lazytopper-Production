...
3. Then in parallel: **PR-F** (`<Note>` component + Topic Hub wiring — cofounder/frontend session, reads `notes/specs`+`notes/assets`, writes `src/`) built on the Light spec, AND **Step-2 spec authoring** (4 prototype enrichments → ~35 notes), validator-gated.
