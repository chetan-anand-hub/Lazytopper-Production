...
## ⏭️ CURRENT POINTER (as of 2026-07-13 — code trunk `f662fbe`, #410 mobile avatar-dropdown on #408 `25c3cd7` arc PR-4 / #409 docs)
...
>
> - **BANK-EXPANSION Batch 8 = quadratic-equations +110 MERGED (#405, trunk `1b7c7aa`).** FIRST Maths topic since real-numbers (pipeline balance); extract-max A/B/C +19 + scarce **D 29→76 (+47, REACHED ≥75)** + case-based **E 22→66 (+44, honest-stop)**. 3 skeptics re-solved every quadratic (1 extract distinctness drop + 2 D correctness fixes + reciprocal-twin trim; E 44/44 clean); the consolidation cross-pack gate caught 3 extracted-C vs authored-D twins → 3 authored D dropped. Owner byte-review CLEAN after 2 corrections: the ~22 sum/product-of-roots items were rightly kept OUT of quadratics but MISLABELLED "Class-11" — they are **Class-10 2026-27 CORE under POLYNOMIALS** (must be extracted next); a suspected complex-roots leak was a FALSE POSITIVE. **Bank → 7,780; 7 distinct topics done; 19 remain (11 Maths + 8 Science).** **⏭️ NEXT = bank-expansion Batch 9 = polynomials** (Maths) — and it **MUST include the ~22 sum/product-of-roots (zeros–coefficients of quadratic polynomials) items as CORE 2026-27 content**, absorbing what was correctly filed out of quadratics. **CONTINUOUS-PUSH:** the lane is at a CLEAN BOUNDARY — a fresh Fable window resumes straight from `handoff/BANK_EXPANSION_LANE_STATE.md` + the task file (regenerate the per-topic census from a fresh dump vs the 7,780 bank; anchor every syllabus call to official CBSE 2026-27 + live `syllabusGuard.ts`, NEVER memory/prior-year). Keep shipping one topic-batch per PR until all 19 remaining are done.
>
...
> - **✅ arc PR-4, the Me/Progress redesign = MERGED (#408, code trunk `25c3cd7`), owner byte-reviewed CLEAN.** The CONSUMPTION layer over PR-B is DONE, desktop + **mobile**: new `pages/mobile/MobileMePage` retires the legacy Streak/XP hero (`pages/app/Me` un-routed for PR-G) → **[FU-MOBILE-ME-PROGRESS-PARITY] closed**; the shared responsive `ProgressWindowArc` now renders every rung (subject/section/topic/concept/mistake-composition, honest-or-silent) on both widths + the honest window empty-state ([FU-PROGRESS-WINDOW-SPLIT-UX] stopgap); `TopicProgressTrend` wires `getTopicProgress` into the Topic Hub → [FU-TOPICHUB-PROGRESS-ARC] wired. Consumption-only (data-layer byte-untouched); careless mark-loss carried forward; BottomNav preserved; trial badge dropped. **⏭️ IMMEDIATE NEXT ON THE CRITICAL PATH = PR-B-v2 (the ENGINE fixes), because owner live-verify found the correctly-wired arc renders EMPTY — 3 PR-B data-layer bugs, NOT arc-PR-4 defects:** [FU-PROG-TOPIC-KEY-MISMATCH] (getTopicProgress matches two different canonicalizers → zero topic rows), [FU-PROG-DATA-COMPLETENESS] (reads the practiceInsights attempts stream only, not sessionRecords → the arc is blind to CT/FT/C&I), [FU-PROG-WINDOW-MODEL] (calendar-midpoint split → activity-median). Fix all three in progressStore (no double-count), then owner live-verifies → the Me/Progress Verified cell flips. THEN Home nudge (arc PR-5). Separately: [FU-MOBILE-CI-PARITY] (C&I mobile parity lane).
>
