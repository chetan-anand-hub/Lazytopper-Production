...
SHA: c5e148d
Last PRs: **#302 Worksheet no-working honesty fix → D-PROG-2 / step 1 CLOSED (ported #301 into `gradeStructuredSet`/`normaliseStructuredResult`: worksheet rule 5 + `noWorkingNulled` guard + `rawAdjusted` reconcile; `handleCheckSolution` byte-identical; MCQ residual ~40% → [FU-WORKSHEET-MCQ-OBJECTIVE-GUARD]; owner dual live-verified; → `c5e148d`)** + **#297 Z3 figure-binding golden slice (113 source figures bound → 93 Qs, rendered as `<img>` in the question body + step-mark pill fix; → `449d686`)** + **#295 Worksheet PR-A grade-results redesign (`1a85186`)** + **#291 Worksheet PR-E2b grade loop (`60c5bf9`)** + **#292 Z3 Competency extraction (102 authentic Maths case-based Qs → `competency.z3.ts`; → `b1d3e46`)** + **#289 Note-spec validator gate (`c525b2a`)** + **#282 Notes track Step-1 (`de2a616`)** + **#280 Worksheet PR-E2a (`d065922`) + #283 PR-E2a.1+.2 (`9a080a0`) + #284 PR-E2a.3 (`cfff277`)** + #279 docs(handoff) post-#278 (`883e904`). Earlier: #272 (**Topic Hub PR-C concept tutor "Teach me" flow**; → `d9ba545`) + #273 (docs(handoff) post-PR-C; → `6aa0640`) + #274 (**Topic Hub PR-D final-IA LAYOUT**; → `b57fa79`) + #275 (docs(handoff) post-PR-D; → `acc419b`) + #276 (**Topic Hub PR-E1 — concept-row Practise → Quick Practice + exact mark-band filter + Chapter-test wired + MockBuilder un-route**; → `1de6f3e`) + #277 (docs(handoff) post-PR-E1; → `b4163ef`) + #278 (**CLAUDE.md governance refresh** — worktree rule, matrix de-hardcode, Replit→CI, CBSE 2026-27, verification doctrine + marks-bucket/MockBuilder/MI/syllabusGuard rules; → `f7170ef`)

## ⏭️ IMMEDIATE NEXT — MCQ-plumbing PR ([FU-WORKSHEET-MCQ-OBJECTIVE-GUARD]) — cofounder-gated, off `c5e148d`
**D-PROG-2 / step 1 is CLOSED for its designed scope (#302):** the worksheet grader's subjective no-working case is now deterministic (empty/whitespace/absent → null + 0 buckets; rawSummary leak → 0; worked-wrong preserved). The **one tracked deterministic gap** is the wrong-MCQ case: `studentWork` is the bare option "(d)" (non-empty → the guard can't fire), so MCQ honesty currently rides on prompt rule 5 and is **non-deterministic (~40% live)**.
- **The fix (clean, decided):** forward `section`/`format` client→server (the signal is dropped today — client `worksheetGradeService.ts` keeps only 7 fields, server keeps only `marks`; `marks===1` is a fragile proxy, NOT used), then apply the **existing `isObjectiveType(qType, section)` helper in `serverUtils.cjs`** (do NOT fork a classifier) in `normaliseStructuredResult` as "incorrect **objective** step → `mistakeType` null, regardless of `studentWork`." Touches the **frontend** + server → bigger than #302's 2-file server-only scope → its own PR.
- **Sequencing:** branches off the post-#302 tip `c5e148d`. **The cofounder hands the full instruction** (it carries its own STEP 0 confirming `isObjectiveType`'s exact definition + the client payload builder before any code). **Do NOT open the MCQ worktree until handed that instruction.** **MUST land before PR-B.**
- Also tracked (separate PRs): **[FU-WORKSHEET-NONATTEMPT-TEXT]** (explicit "don't know"/non-attempt text — non-empty, guard can't see it), **[FU-WORKSHEET-BANK-ANSWER-POLLUTION]** (~26 files / ~54 strings marking-scheme junk in model answers; content lane).
