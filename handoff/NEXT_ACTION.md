...
SHA: 686f737
Last PRs: #251 (AI-tier PR1 mechanical content-integrity; → `f4a41b6`) + #252 (docs) + #253 (AI-tier PR1b pack-file 5-mark retags; → `f83915b`) + #254 (docs) + #255 (AI-tier PR2a source-provenance stamp + soft AI-lower ranking; → `686f737`)

## ⏭️ IMMEDIATE NEXT — AI-tier PR2b (`pastBoardYear` strip) → [FU-AITIER-RANK-MOCKS-HPQ] (mocks/HPQ ranking) → [FU-AITIER-MARKS-MISMATCH] content pass → (iii) gated-spelling → (2) MI eval → (3) Stage 3 → Fix B
...
practice covered, owner live-verify PASS)** are done. The items below are **QUEUED but NOT yet authorized** — the owner sends each
as its own instruction, branched fresh from `686f737`. Do not start until instructed.

- **(NEXT) AI-tier PR2b — `pastBoardYear` strip.** The remaining half of the audit's PR2, now **unblocked** by PR2a's stamp:
  the `_source` tier distinguishes verifiable PYQ years from fabricated predicted-layer ones, so the unverified `pastBoardYear`
  can be stripped from the predicted/AI layers without losing genuine PYQ provenance. NOTE the dedup at `predictionCore.ts:127`
  currently prefers questions WITH `pastBoardYear` — PR2a did not rely on it for tier; PR2b must handle that tie-break. Separate,
  owner-authorized.
- **(THEN) [FU-AITIER-RANK-MOCKS-HPQ] — ranking parity for the remaining surfaces.** PR2a's `sourceMultiplier` only applies inside
