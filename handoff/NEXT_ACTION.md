...
SHA: 444238b
Last PRs: #257 (AI-tier PR2b strip `pastBoardYear`; → `d6e0e14`) + #258 (docs) + #259 (AI-tier FU-RANK-MOCKS-HPQ; → `775ee75`) + #260 (docs) + #261 (docs(design) Topic Hub / Learn-Flow LOCKED spec; → `169a74c`) + #262 (Bank Expansion P1 Batch 1: 60 net-new Exemplar Maths Qs + AI step-marked solutions; → `444238b`)

## ⏭️ IMMEDIATE NEXT — Bank Expansion P1 **Batch 2 (Areas-Related-to-Circles + Coordinate-Geometry)** when owner authorizes
Batch 1 (AP+Statistics+SAV, 60 net-new) is DONE + merged (`444238b`). Same recipe per batch: extract verbatim from the Exemplar PDF
→ syllabus-filter (copy banned list from `scripts/src/syllabusGuard.ts`; Coord-Geom drops **Area-of-Triangle-in-Coordinate-Geometry**)
→ dedup vs repo (by `ncertRef` + content; surface borderline) → AI step-marked solutions (`[N mark]` summing to marks; finalAnswer
cross-checked vs jeep2an.pdf) → new `*.exemplar2.ts` + register in `canonicalQuestionBank.ts` + add ids to `AI_GENERATED_SOLUTION_IDS`
(NEVER edit `predictionTypes.ts`) → gates + Codespaces vitest → **STOP for owner solution/fidelity verification; no self-merge.**
Reusable tooling: `C:\Users\Chetan\OneDrive\Desktop\diff\exemplar-extraction\`.
