...
## ⏭️ CURRENT POINTER (as of 2026-07-13 — trunk `1b7c7aa`, #405 bank Batch 8 on #403 `894ef6a` PR-B / #404 docs)
...
> - **⏭️ IMMEDIATE NEXT ON THE CRITICAL PATH = arc PR-4, the Me/Progress redesign** — reads the merged PR-B engine. **Requirement set from PR-B owner live-verify (all consumption-surface, NOT engine bugs):** [FU-PROGRESS-WINDOW-SPLIT-UX] (clearer wider-window-silent empty state) · [FU-TOPICHUB-PROGRESS-ARC] (wire `getTopicProgress` into Topic Hub) · **[FU-MOBILE-ME-PROGRESS-PARITY] (MOBILE NON-NEGOTIABLE — the arc renders only in `DesktopMePage` ≥1024px; mobile still the legacy `pages/app/Me`; converge onto the one engine)**. Then Home nudge (arc PR-5). Separately: [FU-MOBILE-CI-PARITY] (C&I mobile parity lane).
>
