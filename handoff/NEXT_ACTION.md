...
SHA: f4a41b6
Last PRs: #246 (Check & Improve detect-then-confirm + question photo upload; → `c9404e1`) + #248 (docs) + #249 ("Finish session" scorecard trigger; → `704dcff`) + #250 (docs) + #251 (AI-tier PR1 mechanical content-integrity; → `f4a41b6`)

## ⏭️ IMMEDIATE NEXT — AI-tier PR1b (19-pack retag/quarantine) → PR2 (provenance + ranking) → (iii) gated-spelling → (2) MI eval → (3) Stage 3 → Fix B
topicKey audit (i) + Fix A (#242) + Check & Improve auto-detect (#244) + detect-then-confirm (#246) + (ii) "Finish session"
scorecard trigger (#249) + **read-only AI-tier audit (DONE — `report-ai-tier-audit-2026-06-17.md`)** + **AI-tier PR1 mechanical
content-integrity (#251 — DONE, trunk `f4a41b6`, [FU-MALFORMED-QUESTION] RESOLVED)** are done. The items below are
**QUEUED but NOT yet authorized** — the owner sends each as its own instruction, branched fresh from `f4a41b6`. Do not start
until instructed.

- **(NEXT) AI-tier PR1b — 19-pack `format:"Short"` cleanup ([FU-AITIER-PACK-5MK-SHORT]).** Owner-authorized as a SEPARATE
  follow-up to #251. Retag ONLY the genuine 5-mark long-answers `format:"Short"→"Long"` in the gated `.pack2/.pack3` files and
  **shrink `PACK_5MK_SHORT_BACKLOG`** in `scripts/src/aiTierContentIntegrityGuard.test.ts` accordingly. **QUARANTINE the
  content↔marks mismatches** (e.g. `TG3-056` "Find the value of cosec 60°", `REP2-039` "Name two contraceptive methods" tagged
  5mk) — do NOT relabel those; flag them for a separate content-judgment pass. Needs explicit pack-file scope (`src/data/**`
  gated). Do NOT start until the owner sends the PR1b instruction.
- **(THEN) AI-tier PR2 — provenance + ranking (architectural).** From the audit §6(a): add a `source`/provenance stamp at the
  module boundary, enforce AI-lower ranking in `predictionCore.getAdjustedScore` (today there is NO tier demotion — AI and
  authentic interleave at parity across all four surfaces), and strip the unverified `pastBoardYear`. Separate, owner-authorized.
