...
# Updated: 2026-07-06 (post-PR #338 — **Progress-Journey ARC · PR-1 the session-record DATA LAYER MERGED, trunk `d704b1c`.** The arc's data spine is live: `sessionRecords/{uid}/records/{code}` store + the ONE `progressStore` aggregation reader + durable cross-device `#NN` + `perQuestionRef`; new Firestore collection → owner-deployed rule `dc73360`. **BUILD LANE IMMEDIATE NEXT: the Universal `<ResultsScorecard>` — now unambiguously Progress-Journey arc PR-2** (refactor from `WorksheetScorecard`; the worksheet variant stays behaviour-identical = the non-regression gate; per-surface variants; it WRITES the session record via the new store). **CONTENT LANE:** Light is ship-tracked — the trusted-student QC verifies the 230 authored solutions + 52 flagged diagrams from `docs/light-extraction-review-queue.md`; deferred gdrive leftovers (757 AR 25Q, 821 objective 51Q, 2022-23 PYQ residual, CBSE Practise Papers) are a later 1-mark pass; **Electricity extraction starts ONLY on explicit owner go**, reusing the pipeline in `Desktop\Content\extraction\light\`.)
# Base SHA: d704b1c
