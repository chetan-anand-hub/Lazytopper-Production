...
**Last updated:** 2026-06-22 · **Notes content merged:** #282 Step-1 (squash `de2a616`) + **#289 validator gate (squash `c525b2a`)**; trunk tip `c525b2a`.
...
1. ✅ **DONE (#289, `c525b2a`): `notes/validate_spec.py`** — the 9-rule source-required validator (note-spec schema v1.1): rejects unsourced verbatim/example/NCERT-figure; checks `topic_key` ↔ `topics.ts`, banned keywords via `syllabusGuard.ts` (trap-safe `SURFACE_BANNED_PHRASES`, `//`-comments stripped, word-boundary), mojibake, third_tab/example `kind` shape, source_ledger count. stdlib only, NO bypass flag. CLI `<spec>`/`--all`/`--json`. Committed with `NoteSpec_Schema.md` (schema v1.1) + the validated `specs/light-reflection-and-refraction.json` + 5 negative fixtures + `specs/_test/run_negative_tests.py` self-test. Light VALID; each negative trips exactly one rule. **Run it before committing any spec:** `python notes/validate_spec.py <spec>` or `--all`.
