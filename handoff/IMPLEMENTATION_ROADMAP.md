...
- [x] **#581 (FORBID-1) — the `SolutionChecker` blanket ban replaced by targeted tests.** 2 files. ★★ **The ban was the ENTIRE protection:** the `AUTOGROW` check regexes `EquationInput.tsx`'s **own source**, and **nothing anywhere asserted one line of `SolutionChecker`'s behaviour** — the ban said *"something must not change"* and never said what. ⇒ **a strict increase in protection, not a trade.** `SolutionChecker.tsx` **byte-identical to trunk**; 19 mutations (16 product + 3 gate) **ALL RED**, every restore verified; the `ResultsScorecard` membership check **survives and is proven firing in CI**; scope correctly **not** widened. ★★★ **M2b is the mutation that justifies the doctrine — renaming `--grow` made the negative assertion go VACUOUS and still pass, and only the POSITIVE CONTROL caught it.** ⚠ It also found that **"Lane Overlap" gates nothing** and that the CONV gate's header falsely claimed CI does not run vitest.
- [x] **#580 (GATE-1) — 🛑 P0: the paid AI endpoints now enforce entitlement on the SERVER.** 6 files (5 + the GATE-1b wiring). ★★ **The spec's §1 was incomplete in a way that would have left the P0 OPEN: the effective tier is `applyExpiry(repairInterruptedTrial(...))`, not the stored `tier` field, and the order is load-bearing** — reading the raw field would have served **every expired trial** *and* **locked out every mid-trial student**, and would have re-opened Wave 4's #574 P0 on the server side. ★ **A real 402 from a real HTTP POST against the real `index.cjs`**, firebase-admin swapped before `require` so **no test seam entered production code**. Denials are **never cached**; `step-solution` stays ungated behind a lazy resolver so free paths spend **zero Firestore reads**. **★ OWNER LIVE-VERIFIED on production — a clean three-way discrimination with an account-specific `trialEndedAt`, proving the gate really read Firestore.**
- [x] **#580 / GATE-1b — the CI wiring, and the proof of the gap became the proof of the fix.** 2 edits to `lazytopper/package.json`, by explicit owner allowlist amendment after the subagent **stopped and reported** rather than absorbing it. **A grep of the prior CI log for `entitlement.test.cjs` returned ZERO** — 43 passing tests invisible to CI. ★★ **The control is what makes it evidence:** it grepped the already-wired sibling in the **same before-log, before editing anything**, so the zero was a missing link and not a grep that could never match. `0 → 1`, 43/43 in CI, tail-guard **re-run with its own control case** (front-loading `test:mojibake` turned it RED), and **restored BY SHA, never by `git diff`.**
- [x] **#582 (AUTH-3) — one door, and a verified email.** 10 files. **NO `AuthContext` keys added** (state rides an optional `emailVerified?` on `AuthUser`), **0 of 25 `vi.mock` factories touched**, **`App.tsx` diff ZERO** ⇒ **BATCH-2 is NOT resequenced.** ★★ **It declined §6 and was right: "reduce `SignUpPage` to a thin render" would have deleted the product's only name capture**, forced deleting 7 guard tests and re-opened PR-B2's one-way-door defect — it kept the name field instead, and `App.tsx` still never moves, which was §6's actual objective. Verification learns of success by explicit `reload()` on **three** triggers, **reasoned from the mechanism** (`onAuthStateChanged` does not fire on an in-place mutation). ★★ **16 screenshots found two pre-existing contrast defects invisible to every assertion in the repo.** **★ OWNER LIVE-VERIFIED end to end, including that a wrong password creates NO second account.** ⚠ **The gate is page-scoped, not app-wide** → `[FU-AUTH-VERIFY-NOT-APP-WIDE]`.
- [x] **#579 (SUPPLY-1) — Dependabot version updates + a CodeQL workflow.** 2 files, landed list verified on the **remote**, merged **last** deliberately (CodeQL starts running on every other open PR the moment it lands). **CodeQL proven to have RUN on the FINAL head** (`results=0 rules=87`), and `results=0` **not** sold as a clean bill of health. **`on.push` had to be added** or the Security tab stays empty — **a guard that runs and displays nothing** — ★ **proven by control case.** ⚠⚠ **IT PROVED ITS OWN SPEC WRONG IN BOTH DIRECTIONS:** secret scanning and push protection were **already enabled**; the genuinely disabled settings are **Dependabot ALERTS and SECURITY UPDATES**. **Recorded as a SPEC ERROR CORRECTED BY THE LANE, not a lane finding.** ⚠⚠ **THIS DOES NOT CLOSE THE SUPPLY-CHAIN ITEM** — `dependabot.yml` configures VERSION updates only and delivers **no vulnerability response** until those two owner toggles are on.

**🛑🛑 THE WAVE'S SUBJECT — `MERGED` IS NOT `DEPLOYED`.** GATE-1 merged; **Vercel never built it and Railway did.** For ~2 hours the server enforced a rule the client had no code to explain, and a free student saw the raw string `premium_required` in red — **the exact defect the merged, green, correct §3D was written to prevent.** Caught by **fetching the live bundle and grepping it** (absent from all 65 chunks). ⇒ **two deploy targets build independently from one trunk; a merge confirms neither; confirm by asking the running system, not a dashboard.** ⚠ **Vercel "Redeploy" rebuilds the ORIGINAL commit, not the branch tip**, and branch protection makes **a Deploy Hook the only manual trigger.**

**★★ AND ITS GENERALISATION:** every lane proved *"a green CI run is evidence only about what it EXECUTED"*; the deploy incident proves the sequel — **a merge is evidence only about the repository.**

**⚠ NOT CLOSED, AND LAUNCH-BLOCKING:** `[FU-UPGRADE-MODAL-SELLS-RETIRED-FEATURES]` — the paywall sells **four surfaces the product no longer ships** to a student being asked ₹599; **its fix belongs in GATE-2's spec, not a separate lane.** And `[FU-AUTH-VERIFY-EMAIL-DELIVERABILITY]` — the verification mail lands in Spam on a **blocking** gate; **owner/DNS work, not an agent lane.**

**NOT STARTED, DELIBERATELY — WAVE 5B, planned and NOT dispatched:** **GATE-2** (must **EXTEND** GATE-1's existing 402 branch in `aiClient.ts`, never add a second beside it), **META-1** (a canonical tag pointing at a domain never owned, live today), **PG-1** (server-only; the `index.cjs` unwiring must be in the **same atomic PR** or the server fails to boot on `require`), the **`quality-gate.yml` stale counts** (its own small PR — ⚠ **`CLAUDE.md` is CORRECT and must not be touched**), and **CI-DOCS** (now unblocked; it **SPLITS rather than TRIMS** — the wave-closing docs PR keeps the full bar because it is **this project's only integration run**). ⚠ **BATCH-1b stays BLOCKED behind `FORBID-2`.** ⚠ **Triage the five Dependabot PRs (#583–#587) before any `package.json` or workflow lane.**

---

## 2026-07-31 - ★★ ✅ WAVE 4 **COMPLETE** (#566–#575, trunk `fcdbfa65`) - ten PRs, a launch-blocking P0 and a live mobile crash both fixed, TWO live-verifies owed, THREE lanes specced and deliberately not started

**STAGES COMPLETE:**

- [x] **#566 (AUTH-1) — the offer strip on the sign-in surface.** `OfferStrip` + 15 tests, plus a `pricing.guard.test.ts` no-literal guard that fired independently and named its own subject. ★ **The mobile mirror was the load-bearing part:** the live `Login.tsx` hides the entire brand panel below 1024px (pinned by an existing test), so **the spec's fallback would have been invisible to every phone student** — fixed with a mirror rendered from ONE shared constant. **★ OWNER LIVE-VERIFIED at desktop and phone.** ⚠ It also landed **nine handoff files it did not report** — see the squash finding in `CURRENT_STATE.md`.
- [x] **#567 (DOC-HARDGATE) — the `MONTHLY_INLINE` hard gate written into `NEXT_ACTION.md`.** 1 file, +34/−0, zero price literals. Every element verified against the repo before being written. ★★ **It found a SECOND way the trap fires:** `pricing.ts`'s own doc comment on `MONTHLY_INLINE` asserts *"this one line is the entire switch"*, and **that is false** — `pricing.guard.test.ts` pins the binding, so the one-line change goes red. **An incorrect in-code instruction on the very constant, at exactly the moment someone is closing the cohort under time pressure.**
- [x] **#568 (GUARD-2, restoring #565) — the blindspot suite wired into CI, and the boundary auditor fixed.** `repo_boundary_acceptance.mjs` now enumerates from the git root and imports the **real** `classifyFile` instead of a drifted private copy that was missing **four** lanes. ★★ **The CI grep is the whole verification and it came back PRESENT** — `scope_guard_blindspot_acceptance.mjs ... PASSED (6/6)` and `REPO_BOUNDARY_SUBJECT ... tracked=1715 outside_anchor=334`, **naming 334 files it previously could not see.** ★ **Linux verified by execution**, not argued away. `[FU-GUARD-1-A]` and `[FU-GUARD-1-B]` CLOSED.
- [x] **#569 (AUTH-2) — a zero-attempt student sees a first-session start card.** 4 files, **+18/−0, pure additions.** ★ **Hydration is TRI-STATE, never boolean:** `"loading"` is the initial value and is re-entered on every uid change, **only `"none"` renders**, and a read that throws sets `"some"` — so the card can appear **only after a completed read returning zero rows.** ★★ **A test proves the code works; a chunk proves it ships** — the CI log carried both the suite line and `assets/FirstSession-SPkKUIod.js 9.00 kB`. **★ OWNER LIVE-VERIFIED:** fresh account → card appeared → graded one answer → card vanished. **Name prompt DEFERRED** (`AuthContextType` exposes no `displayName` write path) → `[FU-AUTH-NAME-PROMPT]`.
- [x] **#570 (GUARD-3/PR-2) — 616 legacy mojibake lines in `handoff/` cleared to 8.** 2 files, not 3 (`SESSION_LOG.md` ended byte-identical). Scripted, never hand-edited: strict incremental UTF-8 decode so a legitimate em dash, NBSP or accented character **cannot** be altered; three independent proofs per file (identical ASCII skeleton, preserved ordered non-ASCII run counts, symmetric 618/618 diff). ★★ **Its first blanket pass "fixed" 8 DELIBERATE SPECIMENS quoted inside lessons about mojibake — the lesson was destroyed by the repair — and it caught and reversed itself before pushing.** All 8 restored byte-exactly.
- [x] **#571 (GUARD-3/PR-1) — the cwd-frame sweep, and a SCOPED mojibake gate.** 6 files. ★★★ **It was told to write an ENFORCE-list of three trees and wrote a one-entry EXEMPT list instead**, with everything tracked enforced by default — *because an enumerated enforce-list is the very defect this lane exists to fix: a new top-level tree would default to INVISIBLE.* **A denylist of exempt trees fails safe.** Five mutations including three proving the carve-out is **not a bypass**; the `MOJIBAKE_SCOPE` count line prints on **every** run including a clean one, enforced by its own assertion. ★ **The gate caught its own author** — its draft embedded literal specimens in its fixture and **was fixed rather than exempted.** Also fixed the `scope:guard` bare-invocation auto-detect bug and renamed `anchor_frame_would_miss`.
- [x] **#572 (GUARD-3/PR-1b) — CLAUDE.md's stale 5-suite count, which the file already contradicted.** **Diff exactly one line**; the *"count GROWS / do NOT hardcode"* warning byte-identical. ★★ **The correct line, fifteen lines below the stale one, was already documenting its own past staleness** — the argument for the warning, made by the file against itself. ★★ **It proved BOTH inherited gates live by control** rather than trusting them: #571's `scope:guard` auto-detect resolved `mode=auto:docs` on a bare invocation, and #571's root-`*.md` mojibake enforcement fired at `enforced_hits=1` on an injected sequence in its own edited line, then was restored.
- [x] **#573 (AUTH-2-FU) — one card, one greeting, and a search box that lied.** The merge is proven by **count AND position** — `toBe(1)` across all three card testids in four tests, plus two `compareDocumentPosition` tests, ★ *so "replaced the empty state" is proven rather than "one card exists."* Hydration untouched and byte-identical. ★★★ **It caught its own predecessor faking evidence:** the prior session's "with-attempts" screenshots were **the same state photographed twice**, because the seeded rows lacked `topicKeys`/`questionIds` and `isSessionRecord` silently filters them. ★ **It kept a Lane F doctrine guard rather than deleting it to make a copy string fit.**
- [x] **#574 (P0-TRIAL) — 🛑 the trial no longer downgrades during its own activation.** 2 files. **The reproduction came FIRST and went red on unmodified trunk.** The fix is one argument — `snap.data({ serverTimestamps: "estimate" })` — at the read-back boundary; ★ **an absent field is still absent under `"estimate"`, so SEC-2's Route C is untouched**, and `applyExpiry` is **unchanged**. A clock-skew path gets a 5-minute tolerance **with a clamp that stops the tolerance being a grant.** `repairInterruptedTrial()` self-heals on every cloud read, provably not client-forgeable. **All four mutations fired**, including M2 as the security proof. ★★ **SEC-2's own 17-test entitlement suite runs GREEN in the same CI run.** ⚠ **LIVE-VERIFY OWED.**
- [x] **#575 (HOTFIX-MOBILE) — 🛑 the hook count is stable across the auth restore on `/browse`.** 2 files. A `useState` sat **below `if (!user) return null`** in `MobileAccountMenu.tsx`, introduced by **#554**; the persisted key is **Firebase auth persistence (IndexedDB)**, not the subscription cache. **Three brief premises were wrong** — not email-only, the SEC-2 lead was unrelated, and "version-stamp vs defensive read" was a false choice (there is no old cache shape; the defect is a hook-order invariant). ★ **`WHERE ELSE?` done properly** — every non-test file under `src/` scanned, 3 hits, 2 verified false positives. ★ **It refused to let a dead matcher stand as evidence:** `react-hooks/rules-of-hooks` reports "0 violations" while running nothing, **control-proven.** ⚠ **LIVE-VERIFY OWED — and only one path proves it.**

**★★ THE WAVE'S SUBJECT — ONE finding, four instances, not four incidents:** `check:mojibake` blind to `handoff/` (green for months over 616 corrupt lines); two acceptance suites CI never ran (0 log matches); `react-hooks/rules-of-hooks` reporting "0 violations" while failing to run at all; and a GitHub branch ruleset that was Active with *"Block force pushes"* enabled and **exempted the only person who could trigger it.** ⇒ **a check that cannot be shown to have looked, and to be capable of failing, is not coverage — it is the APPEARANCE of coverage, which is worse, because it stops anyone looking.**

**⚠ TRUNK WAS REWRITTEN MID-WAVE.** A force push from the shared checkout dropped #564 and #565 five seconds after a fetch satisfied `--force-with-lease`. #565 was re-landed as **#568**; **#564's content was restored in full by #566's squash and is verified whole on trunk.** See `CURRENT_STATE.md`.

**NOT STARTED, DELIBERATELY — specced with files on disk:** `AUTH-3`, `BATCH-2`, `NAME+LINK`. ★ **Starting a lane at 40% context and stranding it mid-way is the failure this operating model exists to prevent, and it has already cost this project three agents.** Left whole for a fresh controller. **`BATCH-1` is BLOCKED on an owner ruling** (client transport option A/B/C; whether its M2 is rewritten or dropped) — its findings are in `NEXT_ACTION.md` and must not be re-derived.

---

## 2026-07-29 - ★★ ✅ WAVE 3 **COMPLETE** (#557–#563, trunk `25e995a7`) - four lanes, seven PRs, rules DEPLOYED and Console-verified, ONE live-verify owed

**STAGES COMPLETE:**

- [x] **#557 (D1) — one-time phone-linking nudge on both Home pages.** `LinkPhoneNudge` + 14 tests; a per-browser distinct-session counter (`lazytopper.homeVisits.v1` + `lazytopper.homeVisit.counted.v1`). **The returning-session signal the spec assumed existed did not exist** — ~170 localStorage keys enumerated, all three candidates traced to their writers and rejected. **Live-verify still OWED** (`hasPhoneLinked` against a real phone-linked account).
- [x] **#558 (C1) — the grader's blanket FORBIDDEN ban replaced by targeted tests.** 32 tests, `# skipped 0`; `checkSolution.cjs` byte-identical to trunk. **Both** gates amended in the #519 shape (commented lift + asserted replacement), each with its **own** replacement checks. Convergence 97→100/100, overlay 34→36/36. Declared deviation: `lazytopper/package.json`, to wire the test into `test:matrix:all` — **a test nobody runs guards nothing** — and both gates now assert that wiring. 13 mutations RED.
- [x] **#560 (GUARD-1) — `scope:guard` sees the whole repo.** All enumerations run from `GIT_ROOT`; new `SCOPE_GUARD_SCOPE:` output line with an independent `anchor_frame_would_miss` re-run; `firestore` + `repoRoot` lanes wired into `classifyFile` **and** `laneBuckets`. 17 unclassifiable top-level paths → zero. `agent3_uiux_guard` repaired (existence guard, dead check removed, zero-match meta-assertion) but **deliberately not wired** — that needs `lazytopper/package.json`. **⚠ Its protections are real but UNENFORCED until `[FU-GUARD-1-A]`/`[FU-GUARD-1-B]` land.**
- [x] **#561 (SEC-1-REV) — Route A closed: the Firestore premium self-grant path.** Emulator-verified `# pass 14 # fail 0 # skipped 0`; nested wildcard block deleted; `request.resource.data.get('tier','')` so partial merges are not denied by a rules error. Two forced allowlist extensions (root `package.json`, `.gitignore`). **DEPLOYED and read back from the Firebase Console.** *(An earlier attempt at this lane — SEC-1 — was ABANDONED, not rebased: its spec's fix was proven inert. That proof is the lane's most valuable artefact.)*
- [x] **#559 (C2) — `responseSchema` (constrained decoding).** THREE parsers ⇒ THREE schemas; Schema C deliberately does not require `annotatedSteps`; `mistakeType` **nullable with no enum**. 50/50 + 29/29, `# skipped 0`, 6 mutations RED. `tutor.cjs` **deliberately left unconstrained**, with reasoning. **★ OWNER LIVE-VERIFIED on production, including the null path.**
- [x] **#562 (D2) — the retired Home page and the dead AI learning path deleted.** 7 files, **+37/−1107**. `pages/Home.tsx` (and its fabricated JSON-LD social proof) gone, `generateAILearningPath` gone, the two stale reCAPTCHA comments in `Login.tsx` corrected. Declared deviation: `pricing.guard.test.ts` probe **repointed, not dropped**, to keep it proving the walk recurses.
- [x] **#563 (SEC-2) — Routes B and C closed: entitlement is not forgeable.** Trial expiry **derived** from an immutable server-set `trialStartDate`; `trialEndDate` removed from every read and write **and** forbidden by rules; delete denied. Rules 27/27 on a real emulator, client 27/27, four required mutations RED plus three isolating one clause each. **DEPLOYED and Console-verified with a fresh student.**
- [x] **EV-1 — the pre-merge evidence lane (read-only).** Zero-skip proof read from the CI logs; 8 D1 screenshots; four spec corrections. ⚠ **Its report file was never written** — `handoff/WAVE_STATE_WAVE3_ARCHIVE.md` is its only record, and it is committed with this handoff. ⚠ **It is committed under the ARCHIVE name deliberately.** The controller's live state file is `handoff/WAVE_STATE.md`, which is **untracked scratch memory, now holding WAVE 4**, and its own header says it *“must never appear in a product PR.”* The bytes committed here are the Wave 3 file, verified **md5-identical** to the controller's own `WAVE_STATE_WAVE3_ARCHIVE.md`.

**NOT SHIPPED, DELIBERATELY:** a thinking budget on the grading call (`gradingGenConfig` still sets no cap — recorded as deliberate so a future budget is a conscious change rather than an accident); any constraint on `tutor.cjs`; server-side entitlement; the wiring of GUARD-1's two new suites into CI.

**★★ THE ONE CLAIM THIS WAVE MUST NOT OVERSTATE.** All three **client-side** routes to free premium are closed in production. **The API server still checks rate limits, not plan** — a student with a valid Firebase token who calls the endpoints directly still reaches paid features regardless of tier. **Paid features are protected IN THE UI. That is not the claim "paid features are protected."** → `[FU-VERIFY-UID-ON-AI-ENDPOINTS]`.

**NEXT STAGE:** the combined **`[FU-GUARD-1-A]` + `[FU-GUARD-1-B]`** lane (both HIGH; both need `lazytopper/package.json`, which no lane's allowlist covered — which is how they slipped once already). Then `[FU-VERIFY-UID-ON-AI-ENDPOINTS]`'s server half, then `[FU-EFF-THINKING-BUDGET]` **after a week of real data, at p90 per marks-band** — never from the current n=2 sample.

**★★ Reusable finding — mutation-verify the SPEC, not only the code.** Three separate specs this wave proposed a fix that did nothing, and each was caught by running the battery against the **proposal**: the SEC-1 rules fix (emulator output byte-identical to the permissive rule), the GUARD-1 `git diff` re-framing (already fixed under `[D47]`/`[D41]`), and the C2 mutation target (a harness that mocks the very call the schema rides on). **All three were the spec author's own, and all three were self-reported.** Counting the cofounder's unmatched verification command, the owner's rollback backup and the stale-checkout deploy, **six proposed mechanisms were proven inert this wave** — one class: *something that reports success while inspecting or changing nothing.*

**★★ Reusable finding — a mutation that does not go red is first a claim about the MUTATION.** C1's M5 ran 32/32 green because its anchor string exists **twice at different indentation** and patched the wrong near-duplicate grader. An anchor matching exactly once proves nothing about *which* occurrence it matched.

**★★ Reusable finding — a deploy is not a merge, and it sits below every gate this project owns.** A `firestore:rules` deploy from the shared checkout re-shipped **six-commit-stale** rules and printed `Deploy complete!`. **The two-word difference between `skipping upload` and `uploading rules` is the whole signal.** A lane whose outcome depends on a deploy closes on the **deployed** state.

**★★ Reusable finding — tightening a write rule breaks every over-sending writer, silently.** SEC-2's `saveCloud` spread `status` into its payload; the new rules refuse two of those fields, so **every cloud write would have failed inside an existing `catch {}`** — the security fix quietly disabling subscription persistence. Enumerate every writer and check what each **actually sends**.

**★★ Reusable finding — a test fixture can encode the defect it is meant to guard.** `activeTrial(5) = {start: now, end: now+5d}` asserted a five-day trial that began today — **a state the product cannot produce, expressible only because the trial length was a stored, writable field.**

**★ Process finding — the evidence lane must close BEFORE the merge.** In the first pass both PRs merged before EV-1 returned. The evidence was retrospectively clean, so nothing needed reverting, but as it stands the evidence lane is an **audit, not a gate**. Open question.
