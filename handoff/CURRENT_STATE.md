...
Last updated: 2026-07-05 (post-PR #331 — **Grading-path bugs: Bug 2 (C&I grade-wipe) + Bug 3 (grader ECF marking) FIXED & OWNER-LIVE-VERIFIED; Bug 1 (progress-sync auth) MERGED but feature DEFERRED → trunk tip `2484cff`**. ONE PR, three file-disjoint fixes (owner directive), built in an isolated worktree off the **re-derived origin tip `bfe8e7e`** — the shared local checkout was **22 commits stale** (at `#306`); do NOT proceed from stale local HEAD. **Bug 2 — C&I grade no longer flashes-then-vanishes:** the MI recording (`recordMistake`/`recordAttempt`, incl. `multiQuestionToCsr`) ran inside the grade `try` whose `catch` shows "Grading unavailable" → a recording throw wiped the shown grade; fix decouples recording into its OWN inner try/catch on all 4 paths (desktop+mobile, multi+single) + adds the missing `save-failed` render branch; the outer catch still surfaces genuine grade failures. Owner LIVE-VERIFIED (multi-Q grade persists with honest "pending" on unreadable pages). **Closes [FU-MULTIQ-CI-GRADE-THROW]. Bug 3 — deterministic ECF partial credit:** prompt-only ERROR-CARRIED-FORWARD MARKING clause appended to Rule 4 in BOTH grader functions (`handleCheckSolution` + `gradeStructuredSet`) — correct method applied to a carried-forward wrong value earns its method marks, only the final-answer mark withheld, single-mark→0, ½-units; NO JS logic changed. Owner LIVE-VERIFIED (cascading quadratic → **2/3** with a proper "Error Carried Forward" step + ½-mark deduction, previously 0.5-vs-1.5). **Closes [FU-GRADING-RELIABILITY] (partial-credit variance). Bug 1 — progress-sync auth fix MERGED, feature DEFERRED (NOT a regression):** `userProgress.cjs` resolves the uid from a verified Firebase Bearer token (X-User-ID fallback) + `dbSyncService.ts` attaches it — the correct fix for the proxy stripping `X-User-ID` (verified: the deployed `artifacts/api-server` strips `x-user-id` but forwards `Authorization`); `questionReport.cjs` left **BYTE-IDENTICAL** (owner call — report auth is already handled at the api-server gateway via `requireFirebaseAuth` + trusted x-user-id injection). Live-verify surfaced the real remaining blocker is NOT auth: progress sync now returns **503 "Database unavailable" — `DATABASE_URL` unset**, a legacy backend-Postgres path the **Progress-Journey arc supersedes → do NOT provision a DB; logged [FU-BACKEND-DATABASE-URL-UNSET]** (supersedes [FU-XUSERID-PROXY-STRIP]). 5 files; forbidden files untouched; gates GREEN (tsc; root **181/181**; lazytopper ops **6/6** incl. llm-path 5/5; mojibake; scope:guard product; `node --check`; `git diff --check`); build + graderEval + vitest are CI/Codespace/owner gates; 3 adversarial reviewers clean. Orchestrated (3 file-disjoint subagents + 3 skeptic reviewers). Owner byte-review PASSED + squash-merged #331 → `2484cff`; no self-merge. Report: `report-grading-path-bugs-2026-07-04.md`. **IMMEDIATE NEXT unchanged: the Universal `<ResultsScorecard>`** (step 3, spec `LazyTopper_Universal_Scorecard_Spec_2026-06-25.md`). ) Previously (post-PR #326 [DRAFT, awaiting owner] + #324/#325 — **Notes track advanced: PR-F `<Note>` render (#324) + quadratic-equations spec (#325) MERGED → trunk `52dd77b`; Light note-spec COMPLETION opened as DRAFT #326**. #324 (`9c7fa81`) — the React `<Note spec={…}/>` renders note-specs in Topic Hub via `import.meta.glob`, honest empty state for spec-less topics; `generated`-bucket figure d3 render deferred → placeholder. #325 (`52dd77b`) — `notes/specs/quadratic-equations.json` (NCERT-verbatim, VALID 9/9, 6 examples incl. NCERT Ex 4.1 Q2(ii), real mindmap, generated figure). #326 (DRAFT, `feat/notes-light-complete`) — completes [FU-NOTES-LIGHT-COMPLETE]: LIFTS the 3 figures (base64 PNG→WebP q82 → `notes/assets/light/{fig_97b,fig_910,fig_99}.webp`) + the mindmap (D3 tree → `spec.mindmap {root,branches}`) from the owner-approved prototype `light_note_ENRICHED_v2` into the Light spec — NO PDF re-extraction; validator VALID 9/9, negative suite OK; spec diff surgical (mindmap +46/−1); `notes/` only; owner-merged content lane, no self-merge. **Topic Hub Notes now RENDERS (Light + Quadratic); ~30 chapters remain** (spec authoring, parallel DATA track). New follow-ups: [FU-NOTE-PDF-EXPORT] (port the prototype's `window.print` Download-PDF into the React `<Note>`, PR-F lane), [FU-NOTE-GENERATED-FIG] (render `generated`-bucket figures — d3 deferred in PR-F, so the quadratic discriminant figure shows a placeholder), [FU-NOTES-LIGHT-COMPLETE] (this — close on #326 merge). Content PR report: `report-notes-light-complete-2026-07-03.md`. ) Previously (post-PR #322 — **Firestore undefined-field persistence fix MERGED → trunk tip `706cc12`; PR-B (#321) now genuinely LIVE end-to-end**: root-caused and fixed the silent Firestore write failure that made PR-B non-functional despite being merged. **Root cause (confirmed live):** `firebaseClient.ts` initialised Firestore with `getFirestore(app)`, which does NOT set `ignoreUndefinedProperties`; the Firestore JS SDK therefore THROWS `"Unsupported field value: undefined"` on any document carrying an `undefined` field. Every attempt doc carries `undefined` (`bloomSkill` on the C&I/MCQ paths, `topicName` when absent), so ALL three attempt writes — the `practiceInsights/{uid}` blob (`saveInsights`), the PR-B `practiceInsights/{uid}/attempts/{id}` subcollection (`recordAttempt`), and the `learnerProgress` attempts segment — threw and were SILENTLY swallowed by fire-and-forget `.catch(() => {})`. localStorage worked (`JSON.stringify` drops `undefined`) and `recordMistake` worked (no `undefined` fields); that asymmetry was the whole bug, and it predated PR-B (the blob write never worked; PR-B inherited it). **Fix (2 files, additive/surgical):** (1) `firebaseClient.ts` — `getFirestore(app)` → `initializeFirestore(app, { ignoreUndefinedProperties: true })` (the root fix; `firebaseClient` is the SOLE Firestore init — verified `getFirestore`/`initializeFirestore` appears in no other file — so this is the first Firestore call and cannot throw "already started"); one global setting drops `undefined` fields across ALL collections instead of throwing. (2) `practiceInsights.ts` — un-muted the two write `.catch(() => {})` to `console.warn(...)` so the next silent Firestore rejection is VISIBLE. No change to Firestore paths/payloads/guards, PR-B's write logic, `firestore.rules`, the grader, or any UI. Gates GREEN: tsc; root matrix **181/181**; lazytopper ops matrix; mojibake; `git diff --check`; exactly 2 files. **CI `quality-gate` GREEN** (1m20s, linux `vite build`) + Vercel deploy pass. **Codespace vitest (Node 22): 190 pass / 3 fail — the 3 failures are ALL pre-existing** in `worksheet/worksheetPdfExport.test.ts` (jsPDF/jsdom mock: `pdf.addImage is not a function`), PROVEN by reproducing the identical 3 on the base commit `c5b4de6` before any edit; unrelated to this change. A unit test CANNOT reproduce this bug — the in-memory Firestore mock accepts `undefined`, unlike real Firestore, which is exactly why it went undetected for weeks. Cofounder byte-reviewed. **OWNER LIVE-VERIFIED end-to-end on production:** after a fresh graded attempt, `practiceInsights/{uid}/attempts` now writes and appears in the console; the durable attempt record carries all fields (subject, topicKey, marksScored/Available, `mode:"graded"`, correct, difficulty, questionId, timestamp); a repeat grade shows NO duplicate (PR-B idempotency, now actually exercised); `learnerProfiles/{uid}/mistakeLogs` still writes (regression clean). **PR-B was merged (#321) but non-functional until this fix — it is now genuinely live.** This docs handoff ALSO sweeps the never-done #321 PR-B docs handoff. New D-register entries **D32** (Firestore init contract — never revert to `getFirestore`/bare `.catch`) + **D33** (a new top-level collection doesn't show in the Firestore console left-nav until reload — "can't see it" ≠ "write failed"). New follow-up **[FU-PROGRESS-SURFACE-BREAKDOWN]**; separate still-open items logged: [FU-XUSERID-PROXY-STRIP], [FU-MULTIQ-CI-GRADE-THROW], [FU-MCQ-ATTEMPTS-NOT-RECORDED], [FU-GRADING-RELIABILITY] (new cross-run partial-credit variance — same solution scored 0.5 vs 1.5 on two runs; grader determinism, not a device fork). **IMMEDIATE NEXT = the Universal `<ResultsScorecard>`** (step 3, spec `LazyTopper_Universal_Scorecard_Spec_2026-06-25.md`). ) Previously (post-PR #319 — **MCQ `correctOption` — deterministic worksheet MCQ scoring (code side) MERGED → trunk tip `a71c81e`**: the [FU-MCQ-ANSWER-OPTION-FIELD] code side. The bank's `finalAnswer` stores answer TEXT ("Snell's law"), not the option letter ("(a)"), so the grader had to INFER which MCQ option matched → MCQ scores were non-deterministic across runs. #319 adds an additive pipeline field so the worksheet grader can do a DETERMINISTIC normalised string compare ("(a)"/"A"/"a" all match) of the picked letter against a bank-supplied correct option. **3 product files + 1 test:** **(1)** `ai/aiClient.ts` — additive optional `correctOption?: string` on `WorksheetGradeQuestionInput`. **(2)** `services/worksheetGradeService.ts` — carry `correctOption` in the client→server mapper (safe cast; not yet in the shared canonical type). **(3)** `server/routes/checkSolution.cjs` — server mapper carries it (after `finalAnswer`), and `normaliseStructuredResult` does the deterministic compare: when the question is OBJECTIVE (`isObjectiveType`) AND `correctOption` is present, it overrides the model's per-step status + marks by a normalised compare (strip parens/brackets, trim, lowercase) — full marks on hit, 0 + full deduction on miss — trusting the deterministic compare over model judgment; `couldNotRead`/empty-`studentWork` steps left UNTOUCHED; the existing `noWorkingNulled`/`rawAdjusted` honesty reconcile runs unchanged after (still nulls `mistakeType` for objective Qs). **SHIPS LATENT:** NO bank entries carry `correctOption` yet, so the field is absent everywhere today and the absent-field path is BYTE-UNCHANGED (falls back to the existing model-judgment + objective honesty guard). Content annotation of MCQ bank entries = a SEPARATE future task that this code unblocks. **Test** (`checkSolutionWorksheetNoWorking.test.ts`, now **13** = 10 existing + 3 new): **(h)** correct pick → status correct + full marks (overrides a model under-award); **(i)** wrong pick → incorrect + 0 marks, `mistakeType` null (overrides a model over-award); **(j)** `correctOption` absent → model-judgment path unchanged (objective guard still nulls the type). Subjective questions and the C&I single-question path (`handleCheckSolution`/`handleDetectQuestion`) BYTE-IDENTICAL; `src/lib/desktop/` + `src/data/` untouched. Built in an isolated worktree off `e2247d2`. Gates ALL GREEN: tsc; root matrix **181/181** (26 suites); lazytopper ops matrix incl. llm-path **5/5**; mojibake; scope:guard product; `git diff --check` clean; exactly 4 files. **CI `quality-gate` GREEN** (1m9s, linux build) + Vercel pass. **Codespace vitest 13/13** (raw per-case output pasted to PR #319 — (a)–(g) existing + (h)/(i)/(j) new). Cofounder **byte-review PASSED**. **OWNER-INSTRUCTED squash-merge → `a71c81e`; branch + worktree deleted.** **LIVE-VERIFY DEFERRED** (no quick live test exists — no bank entries carry `correctOption` yet; the code ships latent) → **[FU-MCQ-CORRECTOPTION-VERIFY]** logged: when the first batch of MCQ bank entries is annotated, run a worksheet with those MCQs ×3 and confirm the score is IDENTICAL across runs — this gates the content-annotation task. **[FU-MCQ-ANSWER-OPTION-FIELD] code side DONE** (annotation pending). **IMMEDIATE NEXT = PR-B — the DURABLE per-student worksheet time-series record** (`AGENT_PR_B_durable_time_series_2026-06-28.md`). ) Previously (post-PR #317 — **Multi-question Check & Improve detect COMPLETE end-to-end MERGED & LIVE-VERIFIED → trunk tip `cd5c8ca`**: three PRs landed the full feature — a student can upload a multi-question question paper (image or PDF) to Check & Improve and grade the WHOLE thing, alongside the unchanged single-question flow. **#315 (`91b5f83`) — multi-question detect + whole-paper grade.** Backend `handleDetectQuestion` (detect path ONLY; grading functions untouched; `thinkingBudget:0` preserved): additive `questions[]` in the prompt + RESPOND schema (each question's number / full text / marks; a single question → a single-item array; the existing `detectedMarks`/`detectedSubject`/`detectedTopic` are unchanged = the first question's values, backward-compatible). `aiClient.ts`: additive `DetectedQuestion` type + `questions?` on `DetectQuestionResponse`. Both pages (`DesktopCheckImprovePage.tsx` + `CheckImprove.tsx`, identical UX): "Upload question(s)" labels; when 2+ questions are detected → a summary chip, the answer input also accepts a PDF, and grading calls the EXISTING `/grade-worksheet` (`gradeStructuredSet`) with the detected set; **CI nomenclature `CI-{S}-{TOPIC}-{NN}`** (device-local sequence) on a compact per-question result (marks + knowledge-gap/careless chips + honest graded X/Y + N pending — pending never scored 0); **MI parity** with the worksheet loop (each legible grade → `recordMistake` + `recordAttempt` on a stable `ci:<code>:q<N>` id; re-grade dedups). **Single-question path BYTE-IDENTICAL.** Test (`geminiThinkingConfig.test.ts`): (d) multi-Q schema+prompt present, (e) the array normalised (marks clamped, textless dropped) with single-Q fields intact. 5 files; gates GREEN; cofounder byte-reviewed; owner-merged. **#316 (`fdadd41`) — detect prompt fix (return ALL questions).** On a real paper, detect was stopping after Q1. Three prompt-only changes to `handleDetectQuestion`: a multi-item RESPOND schema with a 2-item example + a `... one object per question found` ellipsis; the questions instruction moved to AFTER the topic list (recency) + an explicit "List EVERY question you find — do not stop after the first"; `maxOutputTokens` 2048 → 4096 (a full paper whose later questions are 5-mark long-answers can clip Q4/Q5 at 2048). `thinkingBudget:0` preserved. Test (c) `maxOutputTokens` assertion 2048 → 4096 (owner-approved 2-file diff — the test guards the detect call config). Verified: grader eval **5/5** (grader path unaffected — detect-only change); a synthetic 5-question Light-Reflection-Refraction PDF through the REAL merged `handleDetectQuestion` + live `gemini-2.5-flash` → `questions[].length === 5`, stable ×3. Gates GREEN; cofounder byte-reviewed; owner-merged. **#317 (`cd5c8ca`) — per-question marks chip (display-only).** The multi-Q chip now lists each question's marks (`Q1 · 1 mark   Q2 · 2 marks …`) between the subject/topic line and the upload prompt (desktop + mobile, identical content; a `detectedQuestions.map`). 2 files; gates GREEN (tsc; root **181/181**; lazytopper ops matrix incl. llm-path **5/5**); cofounder byte-reviewed; owner-merged. **OWNER LIVE-VERIFIED on Vercel + mobile** — the Light-Reflection-Refraction PDF shows "5 questions detected · Science · Light - Reflection & Refraction", then "Q1 · 1 mark  Q2 · 2 marks  Q3 · 3 marks  Q4 · 5 marks  Q5 · 5 marks", then "Upload your answer sheet below to grade all 5"; same on mobile. **[FU-MULTI-QUESTION-DETECT] CLOSED — multi-question C&I is complete END-TO-END: detect ALL questions · per-question marks chip · whole-paper grade · CI nomenclature · MI parity.** **IMMEDIATE NEXT = the MCQ `correctOption` code side** ([FU-MCQ-ANSWER-OPTION-FIELD]; `AGENT_eval_and_mcq_option_2026-06-29.md` PR-B section — give MCQs a canonical option letter so the grader can deterministically compare the picked letter), THEN **PR-B** the DURABLE per-student worksheet time-series record (`AGENT_PR_B_durable_time_series_2026-06-28.md`). **[FU-GRADE-ANY-WORKSHEET]** remains OPEN (grading external non-LazyTopper papers — a future feature). ) Previously (post-PR #313 — **Grader miscopy fix MERGED & LIVE-VERIFIED → trunk tip `129a73e`**: prompt-only, additive, applied to BOTH grading paths in `lazytopper/server/routes/checkSolution.cjs` and kept in sync — `handleCheckSolution` **rule 15** + `gradeStructuredSet` **rule 9**, identical wording (STEP-0 confirmed the prior last rules were 14 / 8). **THE RULE:** when a student's working is internally consistent and mathematically correct but solves a DIFFERENT equation/expression than the one STATED (they miscopied/misread the question from the paper), award **0 marks for the whole question** and classify `mistakeType` **`silly`** — a correctly-solved WRONG problem earns no credit; tell-tale = the student's coefficients/values don't match the question's. No normalisation/logic change (the model applies the rule; the no-working/objective honesty guard is untouched). Closes **[FU-MISCOPY-CLASSIFICATION]** (live evidence WS-M-MIX-22 Q1: student miscopied 4x²-4x-3 as 4x²-4x-5, solved it correctly, got "Concept gap" — owner ruling = `silly` + 0; the prior rules covered slips in the student's OWN working, not miscopying the question itself). Tests: `checkSolutionGradingReliability.test.ts` +2 — **(j)** the rule present in BOTH prompts (guards the both-functions invariant), **(k)** a miscopied-question answer normalises to 0 + `silly`, surviving the honesty guard (visible working + non-objective → diagnosable type kept). **2 files** (`checkSolution.cjs` + the reliability test). Gates GREEN (Codespace Node 22): tsc; **vitest 13/13** (11 existing + j,k); root matrix **181/181**; lazytopper ops matrix incl. llm-path **5/5** (the first 4/5 was a missing-`ripgrep` Codespace false-negative → `apt-get install ripgrep` → 5/5; unrelated to the change); mojibake; production `vite build` + verifier all-checks-passed; `git diff --check`. Cofounder byte-reviewed (PASS). **OWNER-INSTRUCTED MERGE → squash `129a73e`** (Railway auto-redeploys). **LIVE-VERIFIED 3/3, stable ×3** by driving the REAL merged `handleCheckSolution` (rule 15, wording identical to rule 9) against live Gemini `gemini-2.5-flash` — the same model Railway runs (the worksheet path needs a PDF, so the text path was the faithful proxy; vitest (k) covers the worksheet normaliser): **(1) miscopy** (4x²-4x-3 stated, student solved 4x²-4x-5) → **0/2, silly**; **(2) worked-wrong, no miscopy** (LCM 6&20, student got 30 dropping the 2²) → **1/2, conceptual** (real type + partial, NOT silly-miscopy, NOT 0); **(3) "Don't know"** (HCF 6&20) → **0/2, no type**, not couldNotRead. **[FU-MISCOPY-CLASSIFICATION] CLOSED. IMMEDIATE NEXT = multi-question detect for Check & Improve** ([FU-MULTI-QUESTION-DETECT]; instruction `AGENT_ci_multi_question_detect_2026-06-29.md` queued — backend array response + frontend selection UI so detect reads ALL questions of a multi-question PDF, not just the first), THEN the MCQ `correctOption` code side ([FU-MCQ-ANSWER-OPTION-FIELD]), THEN PR-B (the DURABLE per-student worksheet time-series record). ) Previously (post-PR #311 — **thinkingBudget detect fix (#310) + standalone grader eval harness (#311) MERGED → trunk tip `2bc545c`**): **#310 (`7276d31`) — thinkingBudget detect fix, LIVE-VERIFIED.** `gemini-2.5-flash` is a thinking model and thinking tokens count against `maxOutputTokens`; the detect-question call capped at 400, thoughts (~383) ate the budget → truncated JSON → `extractJsonObjectFromText` null → "We couldn't read the question this time." Two additive changes to the shared gateway path: **(1)** `geminiClient.cjs` now threads an optional `thinkingConfig` into `generationConfig` in BOTH `callGemini`'s `buildBody` AND `callGeminiStream` — **INVARIANT (tested): when a caller passes no `thinkingConfig` (grade, tutor, diagrams, more-like-this, warm-pool — everything else) the outgoing body is BYTE-IDENTICAL to before**; the key is only ever ADDED. **(2)** `checkSolution.cjs` `handleDetectQuestion` sets `thinkingConfig: { thinkingBudget: 0 }` on its `callGemini` (nothing else on that call changes; the reasoning sites — grade @16000, tutor, stepSolution, explain-repairs — stay dynamic, out of scope). 3 files (`geminiClient.cjs` + `checkSolution.cjs` + new `geminiThinkingConfig.test.ts`); no new packages / no lockfile change. Gates GREEN (tsc; root matrix **181/181**; lazytopper ops matrix incl. llm-path 5/5; mojibake; scope:guard product; `git diff --check`); **Codespace vitest 5/5** ((a) both body paths forward `thinkingBudget:0`; (b) both byte-identical when absent; (c) detect site sends 0); CI `quality-gate` GREEN; cofounder byte-reviewed. **OWNER DUAL LIVE-VERIFIED:** Q1.png now reads (marks/subject/topic, no "couldn't read"); worksheet grading + tutor regression CLEAN (they pass no `thinkingConfig` → byte-identical). NOTE: Check & Improve detect reading only the FIRST question of a multi-question PDF is **by-design behaviour, not a bug** → tracked as [FU-MULTI-QUESTION-DETECT]. The LIVE-PROOF diagnostic re-run could not be done by the agent (the borrowed Codespace was recreated — inputs gone, no key in env) → folded into the owner live-verify, which PASSED. **#311 (`2bc545c`) — standalone worksheet-grader eval harness (PR-A), MERGED (cofounder PR).** The [FU-GRADE-EVAL-SCRIPT] follow-up landed: a standalone harness so future prompt-only grader PRs can be verified against FIXED synthetic inputs without the owner generating new worksheets each time. **IMMEDIATE NEXT = the grader miscopy fix** (prompt-only, BOTH grading functions, rule 15 / rule 9 — a miscopied question must score 0 + `silly`, NOT a concept gap; see [FU-MISCOPY-CLASSIFICATION]), THEN the MCQ `correctOption` code side ([FU-MCQ-ANSWER-OPTION-FIELD]), THEN PR-B (the DURABLE per-student worksheet time-series record). New follow-ups: **[FU-MISCOPY-CLASSIFICATION]** (grader misclassifies a miscopied question as a concept gap; should be 0 marks + `silly`; fix queued as the immediate-next prompt PR), **[FU-MULTI-QUESTION-DETECT]** (Check & Improve detect reads only the first question from a multi-question PDF; needs a backend array response + a frontend selection UI; spec in progress), **[FU-GRADE-ANY-WORKSHEET]** (students need to grade non-system-generated papers; depends on multi-question detect). ) Previously (post-PR #308 — **Grading-reliability prompt+config hardening MERGED & LIVE-VERIFIED** (#307 squash `195ecf7` + #308 squash `54c959e` → **trunk tip `54c959e`**): two additive, PROMPT-ONLY PRs to the shared grader `lazytopper/server/routes/checkSolution.cjs`, applied to BOTH grading paths and kept in sync — the single-question `handleCheckSolution` and the worksheet `gradeStructuredSet`. No logic changes; the no-working guard / `noWorkingNulled` / `rawAdjusted` reconcile are untouched. **#307 (`195ecf7`):** **(1)** grading **temperature 0.15 → 0.05** on BOTH grading calls — kills the OCR-cascade variance that swung a correct completing-the-square grade 0/3 ↔ 2/3 between runs (0.05 keeps just enough flex for genuinely ambiguous handwriting). **(2)** legible-**non-attempt exception** — a written "Don't know"/"Dont know"/"I don't know"/"DK" is graded `status incorrect`, full marks deducted, `mistakeType null`, and is NEVER `couldNotRead`; landed VERBATIM on `gradeStructuredSet` rule 6, and as an ADAPTED parallel note on `handleCheckSolution` rule 6 (which has no `couldNotRead` field — STEP-0 discrepancy surfaced, owner chose the adapt-wording option). **(3)** a **word-problem closure rule** (rule 14 / rule 8) — stating which root fits the context (e.g. "N=8, reject N=−20") is a required final step worth ≤½ mark if omitted. **#308 (`54c959e`)** — follow-up from WS-M-MIX-21 live evidence (3 runs gave 3 different totals 2.5/3/3.5): **(A) crossed-out = NO-ATTEMPT** — a clearly & completely crossed-out answer with no replacement → graded incorrect / full-deduction / `mistakeType null`, NEVER `couldNotRead`/pending (`gradeStructuredSet` rule 6 only — no `couldNotRead` in `handleCheckSolution`); **(B) PARTIAL-CREDIT step-weight rule** in BOTH prompts — award strictly by the scheme step weights; a correctly-attempted step keeps its marks even when a later step is wrong; a calc-error step earns 0 for that step only; never redistribute/re-weight; even split when no explicit per-step weight. Tests: `lazytopper/src/services/checkSolutionGradingReliability.test.ts` — #307 added 7 cases (a–e: capture genConfig + prompt text on both paths; temp 0.05; exception + closure rule present; pre-existing honesty invariants unchanged), #308 +4 cases (f: crossed-out rule worksheet-only/absent from check prompt; g: PARTIAL-CREDIT in both; h: crossed-out → graded-not-pending, couldNotRead never fires; i: deterministic 2/3 partial credit). Gates GREEN both PRs (tsc; root matrix **181/181**; lazytopper ops matrix incl. llm-path 5/5; mojibake; scope:guard product; `git diff --check`); **Codespace vitest** #307 7/7 then #308 11/11 + existing grader guards 17/17 (**28/28, no regression**); CI `quality-gate` GREEN; cofounder byte-reviewed both; each diff = `checkSolution.cjs` + the reliability test ONLY. **OWNER LIVE-VERIFIED (both PRs):** "Don't know" → 0 every run (never couldNotRead), crossed-out handled correctly, scores stable (runs 2 & 3 identical; run 1 differs only 0.5 on a genuine borderline partial-credit case — acceptable). **[FU-GRADING-RELIABILITY] CLOSED** (temperature + couldNotRead) and **[FU-WORKSHEET-NONATTEMPT-TEXT] CLOSED** (legible non-attempt + crossed-out). NEXT: the **detect/`thinkingBudget` fix** (`handleDetectQuestion` / `geminiClient.cjs` / `thinkingBudget` — a separate instruction already written), THEN PR-B. New follow-up **[FU-GRADE-EVAL-SCRIPT]** — build a Codespace-runnable Node eval script that calls the live grader with FIXED synthetic inputs, so future prompt-only PRs can be agent-verified without the owner generating new worksheets each time. NOTE: the #307 merge happened mid-flow on the explicit "Merge #307" instruction BEFORE its live-verify; live-verify then ran against deployed trunk and surfaced #308's two fixes, which landed as a fresh follow-up PR off `195ecf7` (a merged #307 cannot be amended). ) Previously (post-PR #305 — **Worksheet MCQ DETERMINISTIC honesty MERGED → D-PROG-2 / step 1 now FULLY deterministic (incl. the MCQ objective case)** (squash `93f1594`): the follow-on to #302 that closes the documented MCQ residual. #302 made no-working honesty deterministic for SUBJECTIVE answers, but a wrong MCQ writes its chosen option ("(d)") into `studentWork` (NON-EMPTY) so the empty-working guard could not fire — MCQ honesty rode on prompt rule 5 and was non-deterministic live (~40%). #305 applies the proven Quick-Practice MCQ principle (G1 — *a bare wrong objective answer has no working to classify → attempt-only, NEVER a fabricated mistake type*) DETERMINISTICALLY by carrying ONE field one more hop and reusing the EXISTING canonical classifier: **(1)** client `worksheetGradeService.ts` now maps `section` into each graded question; `WorksheetGradeQuestionInput` (`ai/aiClient.ts`) gains an additive optional `section?`. **(2)** server `handleGradeWorksheet` keeps `section` (+`format`/`qType` defensively for a future Chapter-Test/Full-Mock poster) on the mapped question (it kept only `marks` before). **(3)** `normaliseStructuredResult` extends #302's no-working pass: for an `incorrect` step whose question is OBJECTIVE (`isObjectiveType(qType||format, section)` — `serverUtils.cjs`, REUSED via a direct acyclic require, NOT forked; true for MCQ/AR/Section-A) it nulls `mistakeType` REGARDLESS of `studentWork` and tallies into `noWorkingNulled` so the `rawAdjusted` reconcile also zeroes any leaked MCQ bucket. Marks/status/totals/attempt UNTOUCHED — only the fabricated type is suppressed; subjective questions untouched; **`handleCheckSolution` BYTE-IDENTICAL**. **4 files** (`checkSolution.cjs` + `aiClient.ts` + `worksheetGradeService.ts` + extended `checkSolutionWorksheetNoWorking.test.ts`, now **10 tests**: new (e) Section-A wrong MCQ "(d)" → null deterministically + marks lost; (f) MCQ + model self-reports conceptual:1 → bucket 0; (g) subjective regression keeps type+marks). Gates GREEN (tsc; root matrix **181/181**; lazytopper ops matrix incl. llm-path 5/5; mojibake; scope:guard product; `git diff --check`); **Codespace worksheet vitest 10/10** (raw pasted to PR #305); cofounder byte-reviewed; **owner LIVE-VERIFIED — MCQ honesty guard confirmed working, all-zero mistake buckets EVERY run** (the ~40% intermittency is gone) → squash-merged #305 → `93f1594`. **[FU-WORKSHEET-MCQ-OBJECTIVE-GUARD] CLOSED.** TWO NEW reliability follow-ups logged from live testing (NOT blockers, do not gate PR-B): **[FU-MCQ-ANSWER-OPTION-FIELD]** (MCQ *scores* still non-deterministic because the bank's `finalAnswer` stores answer TEXT not the option letter, so the grader can't do a deterministic string compare of the picked option — the honesty guard is fixed; the score-correctness path is not) and **[FU-GRADING-RELIABILITY]** (temperature 0.15 causes OCR-cascade variance on borderline partial credit; `couldNotRead` fires inconsistently on legible "Don't know" responses). NEXT: the grading-reliability PR (temperature / detect / thinkingBudget), THEN the detect/thinkingBudget fix, THEN PR-B. Previously (post-PR #302 — **Worksheet no-working honesty fix MERGED → D-PROG-2 / step 1 CLOSED** (squash `c5e148d`): ported #301's no-working honesty fix from the single-question `handleCheckSolution` into the SEPARATE worksheet grader `gradeStructuredSet` → `normaliseStructuredResult` (which never had it). Additive; mirrors #301 exactly; **`handleCheckSolution` BYTE-IDENTICAL**. **(1)** worksheet prompt **rule 5** — a wrong step with NO working (e.g. a bare MCQ option "(d)") is undiagnosable → `mistakeType` null, never guess conceptual; status/marks unchanged. **(2)** `normaliseStructuredResult` += the `noWorkingNulled` guard (`status==='incorrect' && !studentWork?.trim()` → null) + the `rawAdjusted` reconcile (subtract the nulled count from the model summary before the floor max). **STEP-0 ground truth (reproduced LIVE via `gradeStructuredSet` on a synthetic "Q1. (d)"):** a wrong MCQ's `studentWork` is the bare option letter **"(d)" (NON-EMPTY)** so the empty-guard can't fire; and **no objective/section flag reaches the grader** (client `worksheetGradeService.ts` drops `section`/`options`; server keeps only `marks`). **DETERMINISTIC fixes proven** (empty/whitespace/absent no-working → null + 0 buckets + marks preserved; rawSummary leak → 0; worked-wrong keeps type+marks). **MCQ honesty rides on rule 5 (prompt) and is NON-DETERMINISTIC live (~40%: 5 runs = 2 null / 3 conceptual)** → tracked residual; the MCQ-plumbing PR closes it. **2 files** (`checkSolution.cjs` worksheet path + new `checkSolutionWorksheetNoWorking.test.ts`); gates GREEN (tsc, root **181/181**, lazytopper llm-path 5/5, mojibake, scope product, diff-check); **CI quality-gate GREEN**; **Codespace worksheet vitest 7/7**; cofounder byte-reviewed (`handleCheckSolution` byte-identical, mirror exact, 2-file diff); **owner dual live-verified (Maths Mixed Worksheet 16) + squash-merged; no self-merge**. Three residuals tracked: **[FU-WORKSHEET-MCQ-OBJECTIVE-GUARD]** (MCQ option ~40% → plumb `section`/`format` client→server + the existing `isObjectiveType` guard; **MUST land before PR-B**), **[FU-WORKSHEET-NONATTEMPT-TEXT]** ("don't know"/explicit non-attempt text is non-empty → guard can't see it → still tagged Concept gap), **[FU-WORKSHEET-BANK-ANSWER-POLLUTION]** (~26 files / ~54 strings of marking-scheme junk in model answers may skew grading; content lane). Report: `report-grader-worksheet-no-working-2026-06-28.md`. Previously (post-PR #297 — **Z3 figure-binding golden slice MERGED** (squash `449d686`): bound every Z3 SOURCE FIGURE (113 rasters → 93 Qs) to its question and render it in the question body as `<img>`, plus the step-mark pill fix — full write-up in NEXT_ACTION/SESSION_LOG. The brief's premise that the raster `<img>` pipeline existed was WRONG (`VisualExplainer` is HTML-only; `public/visuals` had zero rasters) → the contained raster path was BUILT (authorized frontend). `visualConceptRegistry.ts` (+`questionId`/`MATHS_FIGURE_VISUALS`/`getFiguresForQuestion`, exact id-keyed), `QuestionVisualAid.tsx` (+`questionId` → `<img>`, priority over synthetic SVG), `competency.z3.ts` (93 rows +`visualExplainerId`+`requiresDiagram`; still 102 rows / 6,643 served, no drift), 113 `public/visuals/maths/**` assets. `predictionTypes.ts`/`canonicalQuestionBank` UNTOUCHED. Owner LIVE-VERIFIED + merged, no self-merge. **[FU-Z3-FIGURE-BIND] CLOSED**; EMF backlog **[FU-Z3-EMF-SVG]** (RN-001/002/005-010). Previously post-PR #295 — **Worksheet PR-A — grade-results redesign (presentation only) MERGED** (squash `1a85186`): the worksheet grade UI rebuilt to the LOCKED redesign spec, building on the E2b grade loop. **THE HARD INVARIANT HELD — the grader (`server/routes/checkSolution.cjs` / `gradeStructuredSet`) is BYTE-UNCHANGED** (absent from the PR diff); PR-A only reorganises how the EXISTING grade output is presented. **5 tasks:** **(1)** NEW `WorksheetScorecard.tsx` — an **auto scorecard popup** the moment grading completes (the Quick-Practice session-scorecard auto-appear pattern, rendered as the LOCKED navy overlay), responsive at 1024px **desktop centered modal ↔ mobile bottom sheet**; header = worksheet name + code; big Fraunces `gradedMarksAwarded/gradedMarksTotal`; amber pending strip; **four-type breakdown** aggregated from `results.filter(!couldNotRead)[].mistakeSummary` → **Knowledge gaps** (conceptual+calculation) / **Careless mark-loss** (silly+presentation, "not weak topics"); footer Read (ghost) + Download (primary), ✕/Read/Download all close it; **all-pending** (`gradedCount===0`) → "couldn't read any answers" + both buttons DISABLED. **(2)** `WorksheetGradePanel.tsx` — results restructured into **collapsible per-section tap-to-reveal** expanders (first open, rest collapsed) + a Download/Practise action row + a "View scorecard" re-open. **(3)** NEW `WorksheetGradedPrintDoc.tsx` + `exportGradedWorksheetPdf` — a **branded graded PDF** via the EXISTING `worksheetPdfExport.ts` `html2canvas → jsPDF` + KaTeX path (I factored the render→rasterise→paginate→save core into a shared `renderElementToPdf` so `exportWorksheetPdf` stays behaviour-identical — the cofounder-verified non-regressive refactor); renders the SAME `response` already on screen (no second grade call); pending stays "couldn't read — not graded, not scored 0"; coaching footer; "Marks shown match your on-screen result." **(4)** summary-leak fix (display-only) — `isLeakySummary` suppresses model meta/refusal prose ("I am unable to access the PDF…"), especially the all-`couldNotRead` case. **(5)** nomenclature — `worksheetNomenclature` builds the code `WS-{S}-{TOPIC}-{NN}` (e.g. `WS-M-RN-03`) + friendly name `{Topic} · Worksheet {N}` (MIX multi / FULL full-subject), `#NN` = device-local count from `listStoredWorksheetsLite`; shown on the scorecard, on-screen sheet, and PDF. **Four-type source = `WorksheetQuestionGrade.mistakeSummary` (not invented).** Built in an **isolated worktree** (`feat/worksheet-grade-redesign-pra`); **6 files +1003/−20** (2 new + 4 modified: `WorksheetScorecard`, `WorksheetGradedPrintDoc` NEW; `WorksheetGradePanel`, `worksheetPdfExport`, `worksheetModel`, `worksheetSessionStore`). Gates ALL GREEN: tsc, mojibake 0, scope:guard product, lazytopper ops matrix, root matrix **181/181**, `git diff --check` clean; **CI `quality-gate` GREEN** (1m17s, incl. linux `vite build`); **`checkSolution.cjs` diff EMPTY**; no forbidden files (`predictionTypes.ts`/`vite.config`/`firebase.json`/`App.tsx`/`main.tsx`/`src/data/**` untouched). Cofounder review = clean (grader byte-unchanged, the shared PDF-export refactor non-regressive, presentation-only, scope exactly 6 files). Opened as a **draft**; **no self-merge; owner marked ready + squash-merged (#295 → `1a85186`).** **⚠ OWNER LIVE-VERIFY = PENDING** (the UI/PDF round-trip is unverifiable by static gates): scorecard auto-pops after grading (desktop modal + mobile bottom sheet); four-type breakdown correct; ✕/Read/Download all close it; Read reveals the tap-to-reveal sheet; Download → branded PDF whose marks/pending match the screen; all-pending disables both buttons; name/code on scorecard + sheet + PDF; **Check & Improve still grades (grader non-regression)**. Report: `report-pr-a-worksheet-grade-redesign-2026-06-24.md`. **NEXT: owner live-verify of #295; then PR-B** — the DURABLE per-student worksheet record (Firestore by UID: nomenclature made durable + the seen-set question-uniqueness + the Me/Progress journey + scorecard persistence + the parent/teacher storage foundation, §B6 wellbeing-framing + minor-data-consent constraints). The separate Topic Hub queue (PR-F Notes/Examiner's-tips → PR-G deletions) is unaffected. ) Previously (post-PR #291 — **Worksheet PR-E2b — one-PDF AI grade loop + Mistake-Intelligence wiring MERGED** (squash `60c5bf9`): the SECOND half of the worksheet (E2a foundation already merged) — a real upload → grade → display → MI round-trip. The student uploads ONE PDF of all answers (labelled Q1, Q2 …); it is graded in a SINGLE structured Gemini call against the worksheet's KNOWN scheme keyed Q1…QN (matched BY NUMBER via the printed label instruction, never blind-segmented). **Server (additive, live backend — auto-redeploys on merge):** `server/routes/checkSolution.cjs` gained a surface-AGNOSTIC `gradeStructuredSet` core + `handleGradeWorksheet` HTTP handler + a deterministic stub + a per-question normaliser; **`handleCheckSolution`/`handleDetectQuestion` are BYTE-UNCHANGED** (the only diff "deletion" is the return-object line, extended with `handleGradeWorksheet`) → **zero regression to the live Check & Improve grader** (the PR's single biggest risk, honored). **Honest-failure (anti-fabrication):** a per-question `couldNotRead` flag — an illegible/absent answer is NEVER fabricated into a mark and NEVER folded into a 0; an omitted question is treated as pending, not zeroed. **Trusted marks:** per-question `totalMarks` is always the scheme value (`q.marks`); the model only awards within it; additive-floor `mistakeSummary` reconcile mirrors the wired path. `readJson` body cap raised to 8 MB on THIS route only (a 5 MB PDF base64-inflates past the default 5 MB). `questions.cjs` + `index.cjs` register `POST /api/grade-worksheet` (+OPTIONS/CORS). **Client (additive):** `ai/aiClient.ts` `gradeWorksheet()` + types; `services/worksheetSessionStore.ts` `save/getWorksheetGrade` (revisit); NEW `services/worksheetGradeService.ts` (the testable seam — map-by-number, persist, fan each LEGIBLE result through the SINGLE MI front door `recordMistake` + score-twin `recordAttempt` with a STABLE `ws:<id>:q<N>` id so a re-upload with the same result is deduped — idempotency via the front door's existing dedup, NO parallel layer; the grade core takes its question set as a PARAMETER so Chapter Test / Full Mock reuse it); NEW `components/worksheet/WorksheetGradePanel.tsx` (upload UI, sync progress ~30–60s, per-question results, **honest "graded X/Y + N pending" totals** kept SEPARATE from the worksheet total, MI evidence line) wired into `WorksheetGenerator.tsx`; NEW `worksheetGradeService.test.ts` (Codespaces/doc-only). **Reconciliation:** the task doc said `recordAttempt` did not exist yet — on trunk it DOES (used by `SolutionChecker`) and `WORKSHEET_TRACK_HANDOFF.md` §4 says wire it too, so both are wired; no `[FU-SCORECARD]` needed. Built in an **isolated worktree** (`feat/worksheet-grade-loop`), **rebased onto trunk `2cab012` post-Z3 with ZERO conflicts** (file-disjoint from the Z3 `src/data` merge), 9 files +1201/−10. Gates ALL GREEN both pre- and post-rebase: tsc, mojibake 0, scope:guard product, lazytopper ops matrix, root matrix **181/181**, `node --check` ×3 `.cjs`, `git diff --check` clean; **CI `quality-gate` GREEN** (1m16s, incl. linux `vite build`); no forbidden files (`predictionTypes.ts`/`Welcome`/`DesktopShell`/`main`/`vite.config`/`firebase.json`/`firestore.rules`/`src/data/**` untouched; MI routing internals only CALLED, never modified). Cofounder review = clean (additive-only; grader byte-unchanged; surface-agnostic core; single front door; honest-failure all confirmed). **No self-merge; owner merged (#291 → `60c5bf9`).** **⚠ OWNER LIVE-VERIFY = PENDING** on the Firebase-authorized trunk URL (backend auto-redeploys on merge; the AI round-trip is unverifiable by static gates) — START SMALL (5-Q): right-question mapping + sensible marks; an illegible page → honest "couldn't read Qn" + total shows graded X/Y + N pending (NOT a deflated mark); result feeds Me/Progress + unlocks the MI-enrich toggle; careless (silly/presentation) → careless insight NOT a weakness; knowledge-gap (conceptual/calculation) → weak-area for the right topic; **Check & Improve still grades + feeds MI (shared-grader non-regression)**; re-upload does NOT double-count; phone end-to-end. Report: `report-pr-e2b-worksheet-grade-loop-2026-06-23.md`. **NEXT: owner live-verify of #291; then the worksheet (E2a+E2b) is COMPLETE** → remaining Topic Hub queue PR-F (Notes + Examiner's-tips content) → PR-G (deletions); separately PR-D.1, [FU-CONTEXTUAL-TUTOR-REBUILD], [FU-CHAPTERTEST-PAGE-REDESIGN]. Worksheet follow-ups carried: [FU-ASYNC-GRADING] (large-worksheet grade times — sync now, async deferred), [FU-PITFALL-DATA], [FU-WORKSHEET-PDF-SERVERSIDE]. ) Previously (post-PR #292 — **Z3 Competency extraction MERGED** (squash `b1d3e46`): the EXTRACTION PILOT proving the extract→classify→syllabus-filter→schema→bank→verify pipeline on the cleanest content slice (the `Z3. Competency Based Questions` Maths module of the Foundation Study Package v3). **102 net-new AUTHENTIC competency/case-based Maths questions** in NEW `questionBanks/class10/maths/competency.z3.ts` (`Z3_COMPETENCY_QUESTIONS`), wired into `canonicalQuestionBank.ts` by exactly **ONE import + ONE spread** among the authentic Maths sources (nothing else mutated). **AUTHENTIC tier** — the `.z3` suffix is NOT a `.pack*`, and the array is deliberately ABSENT from `AI_GENERATED_PACK_SOURCES`, so `predictionCore` stamps it `authentic`; **no `pyqYear`** → the non-PYQ **"others"** bucket. **THE DECOUPLE:** QUESTION text transcribed verbatim from the Z3 DOCX (python-docx + ftfy); figure-borne DATA — Statistics frequency tables, Coordinate/Probability diagrams, carton/target dimensions — was READ from the embedded PNGs and inlined (`requiresDiagram:true` on 28 rows); SOLUTIONS are AI-authored, step-marked (`[N mark]` summing to marks), cross-checked vs the Z4 source answer key — **PENDING owner/teacher verification** (EVERY marks value is INFERRED — the source carried no mark tags). **Count reconciliation:** source = **117** numbered Qs (not the doc's "~124"); **15 dropped → 102 kept**. Syllabus drops (question-level, re-read vs `scripts/src/syllabusGuard.ts`): all 10 Polynomials (remainder/factor theorem + cubic/quartic functions — beyond Class 10), 1 complementary-angle trig (Intro-Trig Q2), 3 conversion-of-solids mensuration (well-embankment / canal→field / rainfall→vessel); Real-Numbers HCF solutions REWRITTEN to **prime factorization** (Euclid's algorithm banned); coordinate triangle-area sub-parts dropped within kept questions. Distribution: section **E/Case-Based 57 + C/Short 36 + D/Long 9**; difficulty Easy 4 / Medium 89 / Hard 9. Added **`competencyZ3.floor.test.ts`** — the silent-zero spread guard (every Z3 id reaches the served bank; Z3 contributes exactly 102; every Z3 id stays ABSENT from `AI_GENERATED_QUESTION_IDS`; bank floor) — vitest, Codespaces path. **Fix pass (2nd commit `0e8b1f4`, pre-merge):** normalized the **5 half-mark Case-Based rows** (ARC-002/003, SAV-005, PR-005/007 — the ONLY fraction-tag rows) to clean INTEGER `[N mark]` schemes summing to 4, so an integer ruler agrees; moved every `Z4 / source-key / prints / inconsistent` audit aside OUT of the student-facing `finalAnswer`/`solutionSteps` into `// PROVENANCE` / `// PENDING OWNER VERIFICATION` / `// AUTHORED` line-comments; settled overrides shipped clean (TR-009 → 16 m, PR-004 → 98.7%); 4 disputed rows (QE-003, ARC-004, TG-104, SAV-005) + 3 blank-authored rows (PLE-009, CG-007, SAV-006) stay SERVED but flagged (deliberately NOT added to `WITHHELD_QUESTION_IDS` — the floor test asserts all 102 served). Gates GREEN: tsc, root matrix **181/181** (incl. the syllabus guard over the new file), lazytopper ops matrix, mojibake, scope:guard `--mode product`, `git diff --check`; CI `quality-gate` GREEN (linux build, 1m18s). **Owner-approved + squash-merged (#292, `b1d3e46`); no self-merge** (gated `src/data`). Reports + per-question provenance tables + 119 staged WebP figures live OUTSIDE the repo at `Desktop\Content\extraction\report-z3-competency-extraction-2026-06-23.md` (+ `agent-out\*.report.md` + `z3-figures\`). **PILOT PROVEN → the pattern now scales to the worksheet folders** as a new parallel content track. New follow-ups: **[FU-Z3-TEACHER-VERIFY]** (the 7 flagged rows + every inferred mark + every AI-authored solution need a teacher pass before treated as exam-certain), **[FU-Z3-FIGURE-BIND]** (bind the 28 `requiresDiagram` figures — staged WebP + `diagramDescription` seam, no new asset field yet — to a renderer), **[FU-Z3-SOLUTION-IDS]** (OPTIONAL: register Z3's AI solutions in `AI_GENERATED_SOLUTION_IDS` if an "AI solution" badge is wanted, mirroring the `*.exemplar2` DECOUPLE — left out by task scope). ) Previously (post-PR #289 — **Note-spec validator gate MERGED** (squash `c525b2a`): the notes track's gated-build-order **step 1** — `notes/validate_spec.py`, the anti-fabrication gate that makes the ~35-note parallel fan-out safe (designed to run as a `SubagentStop` hook). **9 rules; reads two LIVE dependencies and never hardcodes them** — `SURFACE_BANNED_PHRASES` from `scripts/src/syllabusGuard.ts` (the trap-safe PROSE list, NOT the question-bank `bannedSubtopics` generics; `//` comments stripped before extraction so quoted words inside a comment aren't mistaken for phrases; word-boundary + case-insensitive, so the syllabus traps hold by construction — *homologous series* IN vs *homologous organs* OUT, *sum/product of roots* IN) and the `slug` set from `lazytopper/src/lib/desktop/topics.ts`. Rules: source-required (definitions both tiers / examples / ncert figures; `formula_strip` source OPTIONAL per v1.1) · `topic_key` ∈ topics.ts · banned-phrase · `third_tab` kind+shape · example kind per subject · mojibake/cid/U+FFFD · `source_ledger` count == sourced fields · figure manifest · structural + figure_ref resolution. CLI `<spec>` / `--all` / `--json`; **NO bypass/force flag** (never force-green). This PR also committed the **schema v1.1 contract** (`notes/NoteSpec_Schema.md`) + the **validated reference spec** (`notes/specs/light-reflection-and-refraction.json`) + 5 negative fixtures + a self-test (`notes/specs/_test/`). Acceptance: Light → VALID (all 9 rules); each negative fixture FAILS on EXACTLY its rule (`run_negative_tests.py` → SELF-TEST OK). **9 files, ALL under `notes/`; content-only; stdlib only.** Gates GREEN + CI `quality-gate` GREEN; owner-merged, **no self-merge**. **→ PR-F is now UNBLOCKED** — `<Note>` builds against the Light spec; Step-2 spec authoring is validator-gated. **NEXT (notes track — gated step 2): a content PR under `notes/`** evolving the kit to `render_note(spec)` + finishing Light's figure (base64→WebP) + mindmap (JS→spec) lift; THEN in parallel **PR-F** (the `<Note>` component + Topic Hub wiring) AND **Step-2 spec authoring** (the 4 prototype enrichments → ~35 notes), validator-gated. Report: `report-validate-spec-2026-06-21.md`. )
